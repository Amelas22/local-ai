...
Examples:
  ...
  %(prog)s --folder-id 123456789 --log-level DEBUG
    ...
    folder_group.add_argument(
        "--folder-id", type=str, help="Box folder ID to process as a single case"
    )
    folder_group.add_argument(
        "--root", type=str, help="Root folder ID containing multiple case folders"
    )
    ...
    parser.add_argument(
        "--skip-cost-tracking", action="store_true", help="Disable API cost tracking"
    )
    parser.add_argument(
        "--save-cost-report",
        action="store_true",
    ...
    if not args.init_shared_knowledge and not args.folder_id and not args.root:
        parser.error(
            "Either --init-shared-knowledge OR one of --folder-id/--root is required"
        )
...
def process_single_folder(
    ...
    try:
        ...
        results = injector.process_case_folder(
            folder_id=folder_id, max_documents=max_documents
        )
        ...
        logger.info(
            f"Folder {folder_id} processing complete: "
        ...
        if generate_timeline and success_count > 0:
            ...
            if case_name:
                try:
                    ...
                    narrative = timeline_gen.generate_narrative_timeline(
...
def process_root_folder(
    ...
    for i, folder in enumerate(case_folders, 1):
        logger.info(f"\nProcessing case {i}/{len(case_folders)}: {folder['name']}")
    ...
    logger.info(
        f"Root folder processing complete: "
    ...
    try:
        ...
        if args.folder_id:
            result = process_single_folder(
                injector, args.folder_id, args.max_documents, args.generate_timeline
            )
        else:  # args.root
