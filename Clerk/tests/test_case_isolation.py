...
from src.models.fact_models import (
    CaseFact, FactCategory, DepositionCitation,
    ExhibitIndex, CaseIsolationConfig
)
...
from src.ai_agents.enhanced_rag_agent import (
    EnhancedRAGResearchAgent, EnhancedResearchRequest
)
...
class TestCaseIsolation:
    ...
    async def test_fact_extractor_isolation(self, case_a_name, case_b_name, sample_document):
        ...
        facts_a = await extractor_a.extract_facts_from_document(
            "doc1",
            sample_document
        )
    ...
    async def test_deposition_parser_isolation(self, case_a_name, case_b_name, sample_document):
        ...
        depositions_a = await parser_a.parse_deposition(
            "depo1.pdf",
            sample_document
        )
    ...
    async def test_exhibit_indexer_isolation(self, case_a_name, case_b_name, sample_document):
    ...
    async def test_timeline_generator_isolation(self, case_a_name, case_b_name):
        ...
        assert len(timeline_data_b.timeline_events) == 0, "Case B has Case A timeline events!"
    ...
    async def test_evidence_mapper_isolation(self, case_a_name, case_b_name):
        ...
        for item in evidence:
            assert mapper_b.validate_case_isolation(item), "Evidence item has wrong case!"
    ...
    async def test_shared_knowledge_access(self, case_a_name, case_b_name):
        ...
        assert len(response_a.florida_statutes) > 0, "Case A cannot access shared statutes"
        assert len(response_b.florida_statutes) > 0, "Case B cannot access shared statutes"
        
        # Verify same statute content
        if response_a.florida_statutes and response_b.florida_statutes:
            assert response_a.florida_statutes[0]["citation"] == response_b.florida_statutes[0]["citation"]
...
class TestCaseIsolationIntegration:
    ...
    async def test_full_pipeline_isolation(self, case_a_name, case_b_name, sample_document):
