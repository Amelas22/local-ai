...
class EmbeddingGenerator:
    ...
    def generate_embeddings_batch(
        ...
        logger.info(
            f"Generated {len(all_embeddings)} embeddings using {total_tokens} tokens"
        )
        return all_embeddings, total_tokens

    def prepare_chunk_for_embedding(
        self, chunk_content: str, metadata: Dict[str, Any]
    ) -> Tuple[Dict, int]:
    ...
    async def generate_embeddings_batch_async(
            ...
            async for attempt in AsyncRetrying(
                ...
                with attempt:
                    try:
                        ...
                        logger.debug(
                            f"Processed async batch {i // self.max_batch_size + 1}"
                        )
                        break

                    except Exception as e:
                        logger.error(
                            f"Error in async batch {i // self.max_batch_size + 1}: {str(e)}"
                        )
                        # Try individual processing for failed batch
                        for text in batch:
                            try:
                                embedding, tokens = await self.generate_embedding_async(
                                    text
                                )
                                all_embeddings.append(embedding)
                                total_tokens += tokens
        ...
        logger.info(
            f"Generated {len(all_embeddings)} embeddings async using {total_tokens} tokens"
        )
        return all_embeddings, total_tokens
    ...
    async def generate_embeddings_concurrent(
        ...
        logger.info(
            f"Generating {len(texts)} embeddings with max {max_concurrent} concurrent requests"
        )
        ...
        logger.info(
            f"Generated {len(embeddings)} embeddings concurrently using {total_tokens} tokens"
        )
