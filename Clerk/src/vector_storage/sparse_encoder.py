...
class SparseVectorEncoder:
    ...
    def build_keyword_sparse_vector(self, text: str) -> Dict[int, float]:
        ...
        logger.debug(
            f"Created keyword sparse vector with {len(sparse_vector)} non-zero elements"
        )
    ...
    def build_citation_sparse_vector(self, text: str) -> Dict[int, float]:
        ...
        all_citations = entities["citations"] + entities["sections"] + entities["rules"]
        ...
        logger.debug(
            f"Created citation sparse vector with {len(sparse_vector)} citations"
        )

        return sparse_vector

    def encode_for_hybrid_search(
        self, text: str
        ...
        logger.debug(
            f"Encoded text to sparse vectors: keywords={len(keyword_sparse)}, citations={len(citation_sparse)}"
        )
...
class LegalQueryAnalyzer:
    ...
    def __init__(self, sparse_encoder: SparseVectorEncoder):
        ...
        # Query type patterns
        self.query_patterns = {
            "citation_search": re.compile(
                r"(?:cite|citation|§|\d+\s+U\.S\.C\.|Rule\s+\d+)", re.I
            ),
            "date_search": re.compile(
                r"(?:on|before|after|between|during)\s+(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec|\d{1,2}[-/]\d{1,2})",
                re.I,
            ),
            "monetary_search": re.compile(
                r"(?:damages|settlement|amount|cost|fee|award).*\$[\d,]+", re.I
            ),
            "definition_search": re.compile(
                r"(?:what is|define|definition of|meaning of)", re.I
            ),
            "precedent_search": re.compile(
                r"(?:precedent|case law|prior cases|similar cases)", re.I
            ),
            "procedure_search": re.compile(
                r"(?:how to|procedure|process|steps|filing)", re.I
            ),
        }
    ...
    def analyze_query(self, query: str) -> Dict[str, any]:
        ...
        analysis = {
            ...
            "recommended_weights": {"vector": 0.7, "keyword": 0.2, "citation": 0.1},
            "filters": {},
        }
