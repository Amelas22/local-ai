...
class DocumentBoundaryDetector:
    ...
    def detect_boundaries(self, pdf_path: str, confidence_threshold: float = 0.7) -> List[DocumentBoundary]:
        """
        ...
        try:
            ...
            final_boundaries = [b for b in final_boundaries if b.confidence >= confidence_threshold]
            
            # Ensure complete coverage
            final_boundaries = self._ensure_complete_coverage(final_boundaries, len(page_features))
            
            self.logger.info(f"Detected {len(final_boundaries)} document boundaries in {pdf_path}")
                ...
                with pdfplumber.open(pdf_path) as pdf:
                    for page_num, page in enumerate(pdf.pages):
                        ...
                        page_feature = PageFeatures(
                            ...
                            has_bates_number=bool(self._extract_bates_number(text)),
                            bates_number=self._extract_bates_number(text),
                            structural_hash=self._calculate_structural_hash_pdfplumber(page)
                        )
    ...
    def _detect_hard_boundaries(self, features: List[PageFeatures]) -> List[DocumentBoundary]:
        ...
        for i, feature in enumerate(features):
            ...
            if i > 0 and feature.has_bates_number and features[i-1].has_bates_number:
                if not self._is_sequential_bates(features[i-1].bates_number, feature.bates_number):
                    confidence += 0.3
    ...
    def _detect_soft_boundaries(self, features: List[PageFeatures]) -> List[DocumentBoundary]:
    ...
    def _detect_page_numbering_resets(self, features: List[PageFeatures]) -> List[DocumentBoundary]:
    ...
    def _ensure_complete_coverage(self, boundaries: List[DocumentBoundary], total_pages: int) -> List[DocumentBoundary]:
        ...
        if boundaries[0].start_page > 0:
            complete.append(DocumentBoundary(
                start_page=0,
    ...
    def _has_footer(self, text: str) -> bool:
        ...
        footer_text = '\n'.join(lines[-5:])
        return bool(re.search(r'Page\s+\d+|©|\d{4}\s+\w+|Confidential', footer_text, re.IGNORECASE))
    ...
    def _calculate_text_density_pdfplumber(self, page) -> float:
        ...
        try:
            ...
            if hasattr(page, 'chars') and page.chars:
                ...
                x_coords = [char['x0'] for char in page.chars] + [char['x1'] for char in page.chars]
                y_coords = [char['top'] for char in page.chars] + [char['bottom'] for char in page.chars]
    ...
    def _has_letterhead(self, text: str, page_num: int) -> bool:
        ...
        matches = sum(1 for pattern in patterns if re.search(pattern, first_part, re.IGNORECASE))
        return matches >= 2
    ...
    def _has_signature_block(self, text: str) -> bool:
        ...
        return any(re.search(pattern, text_lower, re.MULTILINE | re.IGNORECASE) for pattern in patterns)
    ...
    def _calculate_structural_hash(self, blocks: Dict) -> str:
        ...
        for block in blocks.get("blocks", []):
            if block.get("type") == 0:  # Text block
                ...
                bbox = block.get("bbox", [0, 0, 0, 0])
                structure.append(f"{int(bbox[0])}:{int(bbox[1])}:{int(bbox[2]-bbox[0])}:{int(bbox[3]-bbox[1])}")
            ...
            if hasattr(page, 'extract_words') and callable(page.extract_words):
                ...
                for y_pos in sorted(lines.keys()):
                    ...
                    if line_words:
                        ...
                        structure.append(f"{int(min_x)}:{int(y_pos)}:{int(max_x-min_x)}:{int(avg_height)}")
                ...
                for y_pos in sorted(lines.keys())[:20]:  # Limit to first 20 lines
                    ...
                    if line_chars:
                        ...
                        avg_height = sum(c['bottom'] - c['top'] for c in line_chars) / len(line_chars)
                        structure.append(f"{int(min_x)}:{int(y_pos)}:{int(max_x-min_x)}:{int(avg_height)}")
    ...
    def _is_email_start(self, text: str) -> bool:
        ...
        matches = sum(1 for pattern in email_patterns if re.search(pattern, first_lines, re.MULTILINE))
        return matches >= 2
    ...
    def _is_form_start(self, text: str) -> bool:
        ...
        return any(re.search(pattern, first_lines, re.MULTILINE | re.IGNORECASE) for pattern in form_patterns)
    ...
    def _get_bates_range(self, features: List[PageFeatures]) -> Optional[Dict[str, str]]:
    ...
    def _should_merge_boundaries(self, b1: DocumentBoundary, b2: DocumentBoundary) -> bool:
        ...
        if abs(b1.confidence - b2.confidence) < 0.2:
            ...
            if b1.document_type_hint == b2.document_type_hint and b1.document_type_hint is not None:
                return True
