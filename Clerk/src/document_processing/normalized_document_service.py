...
class NormalizedDocumentService:
    ...
    async def create_case(
        ...
        try:
            ...
            self.logger.info(
                f"Created case: {case_name} ({result.id}) in matter {matter_id}"
            )
            return result
    ...
    async def process_document(
        ...
        try:
            async with self._transaction_context(transaction_id):
                ...
                document_response = await self.document_manager.create_document(
                    create_request
                )
                ...
                if enable_fact_extraction and self.fact_extractor:
                    await self._extract_document_facts(
                        document_response.document_core, extracted_text, chunks
                    )
    ...
    async def process_documents_batch(
    ...
    ) -> List[NormalizedDocumentResponse]:
        """
        ...
        try:
            ...
            tasks = [process_single_with_semaphore(doc_info) for doc_info in documents]
    ...
    async def _process_with_retry(
        self, doc_info: Dict[str, Any], config: BatchProcessingConfig
    ) -> NormalizedDocumentResponse:
    ...
    async def search_documents_in_case(
        self,
        case_id: str,
        query: str,
    ...
    async def _extract_document_facts(
        ...
        try:
            ...
            self.logger.info(
                f"Extracted {len(facts)} facts from document {document_core.id}"
            )

        except Exception as e:
            self.logger.error(
                f"Fact extraction failed for document {document_core.id}: {e}"
            )
