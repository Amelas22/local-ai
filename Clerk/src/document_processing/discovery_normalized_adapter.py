...
from ..document_processing.hierarchical_document_manager import (
...
class DiscoveryNormalizedAdapter:
    ...
    async def _create_document_from_segment(
        self,
        ...
        document_metadata = DocumentMetadata(
            ...
            summary="Document extracted from discovery production",
        ...
        await self.hierarchical_manager._store_document_components(
            document_core,
            document_metadata,
            None,  # Junction created separately
        )

        return document_core

    async def _create_segment_relationships(
        self,
        ...
        for continuation_id, group in continuation_groups.items():
            ...
            for i in range(len(group) - 1):
                ...
                relationship = await self.hierarchical_manager.create_document_relationship(
                    ...
                    confidence=min(
                        current_segment.confidence_score, next_segment.confidence_score
                    ),
                )
    ...
    async def _create_case_junctions(
        self,
    ...
    async def convert_unified_to_normalized(
        self, unified_doc: UnifiedDocument, case_id: str
    ) -> NormalizedDocumentResponse:
        """
        ...
        try:
            ...
            self.logger.error(
                f"Failed to convert unified document {unified_doc.id}: {e}"
            )
            raise

    def create_chunk_metadata_from_unified(
        ...
        try:
            ...
            for junction_result in junction_results:
                ...
                if doc_core_results:
                    ...
                    metadata_results = (
                        self.hierarchical_manager.qdrant_store.search_points(
                            collection_name=self.hierarchical_manager.collections[
                                "document_metadata"
                            ],
                            query_vector=[0.0],
                    ...
                    doc_metadata = (
                        metadata_results[0].payload if metadata_results else {}
                    )

                    # Filter by document type if specified
                    if (
                        document_types
                        and doc_metadata.get("document_type") not in document_types
                    ):
                        continue
...
class DiscoveryBackwardCompatibility:
    ...
    async def process_discovery_production_legacy(
        self,
        production_result: DiscoveryProductionResult,
        case_name: str,
        discovery_request: DiscoveryProcessingRequest,
        ...
        try:
            ...
            if not case_id:
                ...
                case_id = await self._create_case_for_discovery(
                    case_name, discovery_request
                )
            ...
            return {"status": "error", "error": str(e), "documents_processed": 0}
    ...
    async def _create_case_for_discovery(
        self, case_name: str, discovery_request: DiscoveryProcessingRequest
    ) -> str:
