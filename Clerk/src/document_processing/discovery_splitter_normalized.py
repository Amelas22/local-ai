...
from .discovery_splitter import (
    BoundaryDetector, DiscoveryDocumentProcessor, DiscoveryProductionProcessor,
    DocumentBoundary, DiscoverySegment, DiscoveryProductionResult
)
from .discovery_normalized_adapter import DiscoveryNormalizedAdapter
...
from .enhanced_chunker import EnhancedChunker, ChunkMetadata
...
class NormalizedDiscoveryDocumentProcessor(DiscoveryDocumentProcessor):
    ...
    async def process_segment_normalized(self,
        ...
        try:
            ...
            document_metadata = DocumentMetadata(
                ...
                title=segment.title or f"Discovery Document - {segment.document_type.value}",
            ...
            if segment.bates_range:
                document_metadata.mentioned_dates.append(f"Bates: {segment.bates_range['start']}-{segment.bates_range['end']}")
            ...
            case_junction = DocumentCaseJunction(
                ...
                bates_number_start=segment.bates_range.get('start') if segment.bates_range else None,
                bates_number_end=segment.bates_range.get('end') if segment.bates_range else None,
                ...
                segment_number=segment.part_number if hasattr(segment, 'part_number') else 1,
                total_segments=segment.total_parts if hasattr(segment, 'total_parts') else 1
            )
            ...
            self.logger.info(f"Created normalized document for segment {segment.segment_id}")
    ...
    async def _process_chunks_normalized(self,
                                       document_core: DocumentCore,
                                       segment_text: str,
                                       segment: DiscoverySegment,
        ...
        try:
            ...
            for chunk in chunks:
                ...
                if segment.document_type in [DocumentType.DEPOSITION, DocumentType.INTERROGATORY_RESPONSE]:
                    chunk.semantic_type = "testimony"
                elif segment.document_type in [DocumentType.EXPERT_REPORT, DocumentType.MEDICAL_RECORD]:
                    chunk.semantic_type = "expert_opinion"
                elif segment.document_type in [DocumentType.POLICY, DocumentType.SAFETY_MANUAL]:
                    chunk.semantic_type = "procedural"
            
            # Store chunks
            await self.normalized_service._store_chunks_batch(chunks)
            
            self.logger.info(f"Created {len(chunks)} normalized chunks for document {document_core.id}")
            
        except Exception as e:
            self.logger.error(f"Failed to process chunks for document {document_core.id}: {e}")
            raise
    
    async def _generate_segment_summary(self, segment: DiscoverySegment, text: str) -> str:
        ...
        if segment.bates_range:
            summary_parts.append(f"Bates: {segment.bates_range['start']}-{segment.bates_range['end']}")
...
class NormalizedDiscoveryProductionProcessor(DiscoveryProductionProcessor):
    ...
    async def process_production_normalized(self,
                                          pdf_path: str,
                                          case_id: str,
                                          discovery_request: DiscoveryProcessingRequest) -> DiscoveryProductionResult:
        """
    ...
    async def _extract_segment_text(self, pdf_path: str, segment: DiscoverySegment) -> str:
    ...
    async def _create_production_relationships(self,
        ...
        try:
            ...
            for continuation_id, group in continuation_groups.items():
                ...
                for i in range(len(group) - 1):
                    ...
                    await self.hierarchical_manager.create_document_relationship(
                        ...
                        confidence=min(current_seg.confidence_score, next_seg.confidence_score)
                    )
            
            # Create exhibit relationships
            exhibit_docs = [(seg, doc) for seg, doc in normalized_documents 
                          if 'exhibit' in seg.title.lower() if seg.title]
            
            for exhibit_seg, exhibit_doc in exhibit_docs:
                # Find potential parent documents
                for seg, doc in normalized_documents:
                    if seg != exhibit_seg and self._is_exhibit_reference(exhibit_seg, seg):
                        await self.hierarchical_manager.create_document_relationship(
            ...
            dated_docs = [(seg, doc) for seg, doc in normalized_documents 
                        if seg.document_type in [DocumentType.CORRESPONDENCE, 
                                                DocumentType.EMAIL_CORRESPONDENCE]]
            
            # Would implement temporal relationship logic
            
            self.logger.info(f"Created relationships for {len(normalized_documents)} documents")
            
        except Exception as e:
            self.logger.error(f"Failed to create production relationships: {e}")
    
    def _is_exhibit_reference(self, exhibit_seg: DiscoverySegment, 
                            potential_parent: DiscoverySegment) -> bool:
...
class DiscoveryBatesIndexer:
        ...
        try:
            ...
            for junction in junctions:
                ...
                if data.get('bates_number_start') and data.get('bates_number_end'):
                    ...
                    try:
                        start_num = int(data['bates_number_start'].replace('DEF', '').replace('PLF', ''))
                        end_num = int(data['bates_number_end'].replace('DEF', '').replace('PLF', ''))
            ...
            index_info = {
                ...
                'productions': list(set(r['production_batch'] for r in bates_ranges if r.get('production_batch')))
            }
            
            self.logger.info(f"Built Bates index for case {case_id}: {len(bates_index)} numbers")
            return index_info
    ...
    async def find_document_by_bates(self, case_id: str, bates_number: str) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            for junction in junctions:
                data = junction.payload
                if self._bates_in_range(bates_number, data.get('bates_number_start'), 
                                      data.get('bates_number_end')):
                    # Get document details
                    doc_core = await self.hierarchical_manager.get_document_core(data['document_id'])
                    doc_metadata = await self.hierarchical_manager.get_document_metadata(data['document_id'])
    ...
    def _bates_in_range(self, bates_number: str, start: Optional[str], end: Optional[str]) -> bool:
