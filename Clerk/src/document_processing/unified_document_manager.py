...
from src.models.unified_document_models import (
    UnifiedDocument, DocumentType, DocumentRelevance, DocumentStatus,
    DuplicateLocation, DocumentProcessingResult, DocumentSearchRequest,
...
class UnifiedDocumentManager:
    ...
    def check_document_exists(self, doc_hash: str) -> Tuple[bool, Optional[UnifiedDocument]]:
            ...
            results = self.client.scroll(
                ...
                scroll_filter=Filter(
                    must=[
                        ...
                        FieldCondition(
                            key="case_name",
                            match=MatchValue(value=self.case_name)
                        )
                    ]
                ),
                limit=1
    ...
    async def process_document(
        ...
        doc = UnifiedDocument(
            ...
            last_modified=datetime.fromisoformat(file_metadata.get('modified_at', datetime.now().isoformat())),
...
Format as valid JSON."""
        ...
        try:
            ...
            classification["document_type"] = DocumentType(classification.get("document_type", "other"))
            classification["relevance_tags"] = [
                DocumentRelevance(tag) for tag in classification.get("relevance_tags", [])
            ]
    ...
    def _pattern_based_classification(self, content: str) -> Optional[DocumentType]:
        ...
        if any(phrase in content_lower for phrase in ['motion for', 'motion to', "plaintiff's motion", "defendant's motion"]):
    ...
    def _extract_author(self, parties: List[str], doc_type: DocumentType) -> Optional[str]:
    ...
    async def search_documents(
        ...
        must_conditions = [
            FieldCondition(
                key="case_name",
                match=MatchValue(value=request.case_name)
            ),
            FieldCondition(
                key="status",
        ...
        if request.document_types:
            must_conditions.append(
                FieldCondition(
                    key="document_type",
                    match=MatchValue(
                        value=[dt.value for dt in request.document_types],
                        any=True
                    )
                )
            )
