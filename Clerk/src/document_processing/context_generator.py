...
Please give a short succinct context to situate this chunk within the overall document for the purposes of improving search retrieval of the chunk. Answer only with the succinct context and nothing else."""
    ...
    def generate_context(
        self, chunk: str, full_document: str, chunk_index: int
    ) -> Tuple[ChunkWithContext, Dict[str, int]]:
    ...
    async def generate_contexts_batch(
        self, chunks: List[str], full_document: str
    ) -> Tuple[List[ChunkWithContext], Dict[str, int]]:
        ...
        logger.info(
            f"Generated contexts for {len(results)} chunks, total tokens: {total_usage['total_tokens']}"
        )
    ...
    async def _async_generate_context(
        ...
        return await loop.run_in_executor(
            None, self.generate_context, chunk, full_document, chunk_index
        )

    def generate_contexts_sync(
        self, chunks: List[str], full_document: str
    ) -> Tuple[List[ChunkWithContext], Dict[str, int]]:
        ...
        try:
            ...
            result = loop.run_until_complete(
                self.generate_contexts_batch(chunks, full_document)
            )
