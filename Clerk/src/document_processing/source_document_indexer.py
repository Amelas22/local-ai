...
Format as JSON."""
        ...
        try:
            ...
            return DocumentClassificationResult(
                ...
                suggested_relevance=[
                    DocumentRelevance(r) for r in classification.get("relevance", [])
                ],
    ...
    def _pattern_based_classification(self, content: str) -> Optional[DocumentType]:
        ...
        if any(
            ...
            "answer" in content_lower
            and "defendant" in content_lower
            and "admits" in content_lower
        ):
        ...
        elif "brief" in content_lower and (
            "appellant" in content_lower or "appellee" in content_lower
        ):
            return DocumentType.BRIEF
        elif "order" in content_lower and (
            "court" in content_lower or "judge" in content_lower
        ):
    ...
    async def index_source_document(
        ...
        source_doc = SourceDocument(
            ...
            author=self._extract_author(
                classification.detected_parties, classification.document_type
            ),
    ...
    def _generate_title(
        self, path: str, classification: DocumentClassificationResult
    ) -> str:
        ...
        # Discovery documents
            ...
            classification.document_type == DocumentType.DEPOSITION
            and classification.detected_parties
        ):
            return f"Deposition of {classification.detected_parties[0]}"

        # Evidence documents
        elif (
            classification.document_type == DocumentType.MEDICAL_RECORD
            and classification.detected_parties
        ):
            ...
            classification.document_type == DocumentType.EXPERT_REPORT
            and classification.detected_parties
        ):
    ...
    async def _store_source_document(self, doc: SourceDocument):
        ...
        # Store in vector database
        point = {"id": doc.id, "vector": embedding, "payload": metadata}

        self.vector_store.client.upsert(
            collection_name=self.source_docs_collection, points=[point]
        )
