...
class EnhancedChunker:
    ...
    def __init__(
        ...
        self.patterns = {
            ...
            "legal_citation": re.compile(
                r"\b\d+\s+\w+\.?\s+\d+|\b\w+\s+v\.\s+\w+|\b\d+\s+F\.\s*\d*d?\s+\d+"
            ),
            ...
            "procedural": re.compile(
                r"\bWHEREAS\b|\bWHEREFORE\b|\bORDERED\b|\bADJUDGED\b", re.IGNORECASE
            ),
        }
    ...
    async def create_chunks(
        ...
        try:
            ...
            self.logger.error(
                f"Failed to create chunks for document {document_core.id}: {e}"
            )
            raise
    ...
    def _determine_chunk_type(self, chunk_text: str) -> ChunkType:
        ...
        if any(
            word in text_lower
            for word in ["therefore", "whereas", "plaintiff", "defendant"]
        ):
            if any(word in text_lower for word in ["facts", "evidence", "testified"]):
                ...
                word in text_lower for word in ["conclusion", "ruling", "finding"]
            ):
    ...
    def _split_large_chunk(
        self, chunk: Dict[str, Any], full_text: str
    ) -> List[Dict[str, Any]]:
    ...
    async def _create_chunk_metadata(
        self,
        document_core: DocumentCore,
        ...
        start_page, end_page = self._calculate_page_boundaries(
            chunk_info["start_char"], chunk_info["end_char"], page_boundaries
        )
    ...
    def _calculate_page_boundaries(
        self, start_char: int, end_char: int, page_boundaries: Optional[List[int]]
