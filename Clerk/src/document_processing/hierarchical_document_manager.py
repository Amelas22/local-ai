...
from ..models.normalized_document_models import (
    ...
    DocumentRelationship, ChunkMetadata, DeduplicationRecord,
    MatterType, CaseStatus, AccessLevel, RelationshipType,
    NormalizedDocumentCreateRequest, NormalizedDocumentResponse,
    HierarchicalSearchRequest, HierarchicalSearchResponse
)
from ..models.unified_document_models import DocumentType, DocumentRelevance
...
class HierarchicalDocumentManager:
    ...
    def _initialize_collections(self):
        ...
        collection_configs = {
            'matters': {
                ...
                'distance': 'Cosine',
                'indexes': ['matter_number', 'client_name', 'matter_type', 'access_level']
            },
            'cases': {
                'vector_size': 1,
                'distance': 'Cosine', 
                'indexes': ['matter_id', 'case_number', 'case_name', 'status', 'effective_access_level']
            },
            'document_cores': {
                'vector_size': 1,
                'distance': 'Cosine',
                'indexes': ['document_hash', 'metadata_hash', 'file_name', 'box_file_id']
            },
            'document_metadata': {
                'vector_size': 1,
                'distance': 'Cosine',
                'indexes': ['document_id', 'document_type', 'document_date', 'ai_model_used']
            },
            'document_case_junctions': {
                'vector_size': 1,
                'distance': 'Cosine',
                'indexes': ['document_id', 'case_id', 'production_batch', 'bates_number_start']
            },
            'document_relationships': {
                'vector_size': 1,
                'distance': 'Cosine',
                'indexes': ['source_document_id', 'target_document_id', 'relationship_type']
            },
            'chunk_metadata': {
        ...
        for collection_name, config in collection_configs.items():
            ...
            try:
                ...
                self.logger.error(f"Failed to initialize collection {full_collection_name}: {e}")
    ...
    async def list_matters(self, client_name: Optional[str] = None, 
                          matter_type: Optional[MatterType] = None) -> List[Matter]:
    ...
    async def create_case(self, case: Case) -> Case:
        ...
        try:
            ...
            self.logger.info(f"Created case: {case.case_name} ({case.id}) in matter {case.matter_id}")
            return case
    ...
    async def create_document(self, 
                            request: NormalizedDocumentCreateRequest) -> NormalizedDocumentResponse:
        ...
        try:
            ...
            if is_duplicate and dedup_record:
                await self._update_deduplication_record(dedup_record, document_core.id, request.case_id)
            elif not is_duplicate:
                await self._create_deduplication_record(content_hash, metadata_hash, document_core.id, request.case_id)
            ...
            self.logger.info(f"Created document: {document_core.file_name} ({document_core.id}) in case {request.case_id}")
            return response
    ...
    async def _store_document_components(self,
                                       document_core: DocumentCore,
                                       document_metadata: DocumentMetadata,
                                       case_junction: DocumentCaseJunction):
    ...
    async def _check_deduplication(self, content_hash: str, 
                                 metadata_hash: str) -> Optional[DeduplicationRecord]:
    ...
    async def _create_deduplication_record(self, content_hash: str, metadata_hash: str,
                                         document_id: str, case_id: str) -> DeduplicationRecord:
    ...
    async def _update_deduplication_record(self, record: DeduplicationRecord,
                                         new_document_id: str, case_id: str):
    ...
    async def create_document_relationship(self,
                                         source_doc_id: str,
                                         target_doc_id: str,
                                         relationship_type: RelationshipType,
                                         context: Optional[str] = None,
                                         confidence: float = 1.0) -> DocumentRelationship:
        ...
        try:
            ...
            self.logger.info(f"Created relationship: {source_doc_id} -> {target_doc_id} ({relationship_type.value})")
            return relationship
    ...
    async def get_document_relationships(self, document_id: str) -> List[DocumentRelationship]:
        ...
        try:
            ...
            self.logger.error(f"Failed to get document relationships for {document_id}: {e}")
            return []
    
    # Search and Query
    
    async def hierarchical_search(self, request: HierarchicalSearchRequest) -> HierarchicalSearchResponse:
        ...
        try:
            ...
            filtered_results = await self._apply_access_control(document_results, request.access_level_required)
            
            # Step 4: Enhance with relationships if requested
            if request.include_relationships:
                filtered_results = await self._enhance_with_relationships(filtered_results)
            
            # Step 5: Cross-case analysis if enabled
            cross_case_patterns = []
            if request.enable_cross_case_analysis:
                cross_case_patterns = await self._perform_cross_case_analysis(request, case_ids)
            ...
            self.logger.info(f"Hierarchical search completed: {len(filtered_results)} results in {search_time:.2f}ms")
            return response
    ...
    async def _determine_search_scope(self, request: HierarchicalSearchRequest) -> List[str]:
    ...
    async def _search_documents_in_cases(self, request: HierarchicalSearchRequest, 
        ...
        for result in results:
            document_results.append({
                'junction_data': result.payload,
                'score': result.score
            })
        
        return document_results
    
    async def _apply_access_control(self, results: List[Dict[str, Any]], 
                                  required_access_level: AccessLevel) -> List[Dict[str, Any]]:
        """Apply access control filtering to search results"""
        # For now, return all results - would implement proper access control
        return results
    
    async def _enhance_with_relationships(self, results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    async def _perform_cross_case_analysis(self, request: HierarchicalSearchRequest,
                                         case_ids: List[str]) -> List[Dict[str, Any]]:
