...
from src.models.fact_models import DepositionCitation
...
class DepositionParser:
    ...
    def _compile_citation_patterns(self) -> Dict[str, re.Pattern]:
        ...
        return {
            ...
            "transcript": re.compile(r"^(\d+):(\d+)\s*([QA]):", re.MULTILINE),
        }
    ...
    def _compile_page_line_patterns(self) -> Dict[str, re.Pattern]:
        ...
        return {
            ...
            "line_number": re.compile(
                r"^(\d+):(\d+)\s+([QA])[.:]?\s*(.+)$", re.MULTILINE
            ),
            # Page header: "Page 45"
            "page_header": re.compile(
                r"^\s*(?:Page|PAGE)\s+(\d+)\s*$", re.MULTILINE | re.IGNORECASE
            ),
    ...
    def _extract_metadata(self, content: str) -> DepositionMetadata:
        ...
        for pattern in date_patterns:
            match = re.search(
                pattern, content[:1000], re.IGNORECASE
    ...
    def _extract_short_form_citations(
        self, content: str, metadata: DepositionMetadata
    ) -> List[DepositionCitation]:
    ...
    def _extract_transcript_testimony(
        ...
        pages = re.split(
            r"^\s*(?:Page|PAGE)\s+\d+\s*$", content, flags=re.MULTILINE | re.IGNORECASE
        )
        ...
        for page_content in pages[1:]:  # Skip content before first page marker
            # Extract Q&A pairs
            qa_matches = list(
                self.page_line_patterns["line_number"].finditer(page_content)
            )
    ...
    async def _store_citations(self, citations: List[DepositionCitation]):
        ...
        for citation in citations:
            ...
            metadata = {
                ...
                "testimony_excerpt": citation.testimony_excerpt[
                    :500
            ...
            points.append({"id": citation.id, "vector": embedding, "payload": metadata})

        # Store in case-specific collection
        self.vector_store.client.upsert(
            collection_name=self.depositions_collection, points=points
        )

        logger.info(
            f"Stored {len(points)} deposition citations in {self.depositions_collection}"
        )
    ...
    async def search_testimony(
        ...
        if deponent_filter:
            must_conditions.append(
                {"key": "deponent_name", "match": {"value": deponent_filter}}
            )
    ...
    def format_citation(
        self, citation: DepositionCitation, style: str = "bluebook"
    ) -> str:
