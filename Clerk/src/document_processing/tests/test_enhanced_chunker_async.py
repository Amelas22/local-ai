...
from ...models.normalized_document_models import DocumentCore
...
async def test_generate_embeddings_batch_async(mock_embedding_generator, document_core):
    ...
    chunker = EnhancedChunker(
        embedding_generator=mock_embedding_generator, chunk_size=1200, chunk_overlap=200
    )
    ...
    chunks = await chunker.create_chunks(
        document_core=document_core, document_text=sample_text
    )
...
@pytest.mark.asyncio
async def test_embedding_fallback_to_individual_async(
    mock_embedding_generator, document_core
):
...
async def test_no_await_on_sync_methods():
    ...
    with patch(
        ...
        chunker = EnhancedChunker(embedding_generator=sync_generator, chunk_size=1200)
        ...
        chunks = await chunker.create_chunks(
            document_core=doc_core, document_text="Test text"
        )
