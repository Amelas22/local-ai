...
Requests 8-10: All documents referenced in your initial disclosures."""
    ...
    def test_request_categorization(self, parser):
        ...
        test_cases = [
            ("All documents, records, and files relating to", RequestCategory.DOCUMENTS),
            ("All emails and correspondence between", RequestCategory.COMMUNICATIONS),
            ("All electronic data and databases", RequestCategory.ELECTRONICALLY_STORED),
            ("All physical objects and samples", RequestCategory.TANGIBLE_THINGS),
            ("All information regarding", RequestCategory.OTHER),
            ("All emails in electronic format", RequestCategory.COMMUNICATIONS),  # Priority test
        ]
    ...
    async def test_parse_rtp_document_success(self, parser, mock_extracted_document):
        ...
        # Setup mocks
        with patch.object(parser.pdf_extractor, 'extract_text', return_value=mock_extracted_document):
            with patch.object(parser.pdf_extractor, 'validate_extraction', return_value=True):
                ...
                try:
                    ...
                    assert req1.category in [RequestCategory.DOCUMENTS, RequestCategory.OTHER]
                    
                    # Check sub-request if found
                    req6a = next((r for r in requests if r.request_number == "6(a)"), None)
                    if req6a:
                        assert req6a.parent_request == "6"
                        assert "safety" in req6a.request_text.lower()
                    
                    # Check roman numeral if found
                    req7 = next((r for r in requests if r.request_number == "VII"), None)
                    if req7:
    ...
    def test_format_detection(self, parser):
        ...
        federal_text = "UNITED STATES DISTRICT COURT\nFRCP Rule 34\nREQUEST FOR PRODUCTION NO. 1"
    ...
    async def test_websocket_integration(self, parser):
        ...
        with patch('src.websocket.socket_server.sio', mock_sio):
            with patch.object(parser, 'parse_rtp_document', new_callable=AsyncMock) as mock_parse:
...
class TestRTPParserPerformance:
    ...
    def test_large_document_performance(self):
        ...
        for page in range(1, 501):
            ...
            if page % 10 == 0:
                large_text += f"Request No. {page//10}: All documents for item {page//10}\n"
