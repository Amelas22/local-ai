...
class TestDiscoveryProductionProcessor:
    ...
    @pytest.mark.asyncio
    async def test_process_discovery_production_basic(
    ...
    def test_sliding_window_processing(self, processor):
        ...
        with patch("pdfplumber.open") as mock_pdf:
            ...
            with patch.object(
                processor, "_detect_boundaries_with_ai", side_effect=track_windows
            ):
                # Act
                result = processor.process_discovery_production(
                    pdf_path="large.pdf", production_metadata={}
                )

                # Assert - Verify sliding window was used
                assert len(window_calls) > 1  # Multiple windows processed
                assert all(
                    len(pages) <= processor.default_window_size
                    for pages, _ in window_calls
                )
    ...
    async def test_error_handling(self, processor):
        ...
        with pytest.raises(Exception):
            processor.process_discovery_production(
                pdf_path="/nonexistent/file.pdf", production_metadata={}
            )
        ...
        with patch("pdfplumber.open") as mock_pdf:
            ...
            with patch.object(
                processor,
                "_detect_boundaries_with_ai",
                side_effect=Exception("AI error"),
            ):
                result = processor.process_discovery_production(
                    pdf_path="test.pdf", production_metadata={}
                )
    ...
    def test_confidence_threshold_filtering(self, processor):
        ...
        # Act
        segments = processor._create_segments_from_boundaries(
            boundaries=boundaries, total_pages=15, production_metadata={}
        )
