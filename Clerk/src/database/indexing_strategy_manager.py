...
from ..models.normalized_document_models import IndexStrategy, QueryPattern, TableStats
...
class IndexingStrategyManager:
    ...
    def __init__(self, qdrant_store: QdrantVectorStore):
        ...
        self.monitored_collections = {
            'legal_matters': ['matter_number', 'client_name', 'matter_type', 'access_level'],
            'legal_cases': ['matter_id', 'case_number', 'case_name', 'status'],
            'document_cores': ['document_hash', 'metadata_hash', 'file_name', 'box_file_id'],
            'document_metadata': ['document_id', 'document_type', 'document_date', 'ai_model_used'],
            ...
            'deduplication_records': ['content_hash', 'metadata_hash', 'primary_document_id']
        }
    ...
    async def analyze_query_patterns(self, 
        ...
        try:
            ...
            self.logger.info(f"Analyzing query patterns from {start_date} to {end_date}")
            ...
            analysis_results = {
                ...
                'performance_summary': self._create_performance_summary(relevant_metrics)
            }
            
            self.logger.info(f"Query pattern analysis completed: {len(recommendations)} recommendations")
        ...
        try:
            ...
            if query_signature in self.query_patterns:
                ...
                pattern.avg_execution_time_ms = (
                    (pattern.avg_execution_time_ms * (pattern.frequency - 1) + execution_time_ms) 
                    / pattern.frequency
                )
    ...
    def _create_query_signature(self,
                              collection_name: str,
                              filters: Dict[str, Any],
                              sort_fields: List[str]) -> str:
    ...
    def _identify_slow_queries(self, metrics: List[QueryMetrics]) -> List[Dict[str, Any]]:
    ...
    async def _generate_index_recommendations(self,
                                            ...
                                            slow_queries: List[Dict[str, Any]]) -> List[IndexRecommendation]:
        ...
        partial_recommendations = self._analyze_partial_index_opportunities(pattern_analysis)
    ...
    def _analyze_partial_index_opportunities(self, 
                                           pattern_analysis: Dict[str, Any]) -> List[IndexRecommendation]:
    ...
    def _deduplicate_and_prioritize_recommendations(self,
                                                  recommendations: List[IndexRecommendation]) -> List[IndexRecommendation]:
        ...
        for group in grouped.values():
            ...
            best_rec = min(group, key=lambda r: (r.priority, -r.estimated_improvement_ms))
    ...
    def _create_performance_summary(self, metrics: List[QueryMetrics]) -> Dict[str, Any]:
        ...
        return {
            ...
            'slow_query_percentage': len([t for t in execution_times if t > self.slow_query_threshold_ms]) / len(execution_times) * 100,
            'collections_queried': len(set(m.collection_name for m in metrics))
        }
    
    async def apply_index_recommendation(self, recommendation: IndexRecommendation) -> bool:
        """
        ...
        try:
            ...
            for column in recommendation.columns:
                self.qdrant_store.create_payload_index(
                    collection_name=recommendation.table_name,
                    field_name=column
                )
    ...
    async def monitor_index_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for index_name, performance in self.index_performance.items():
                ...
                if performance.hit_count > 100 and performance.estimated_benefit_ms > 50:
