...
from .indexing_strategy_manager import IndexingStrategyManager, QueryMetrics, IndexRecommendation
...
class QueryInterceptor:
    ...
    async def _suggest_immediate_optimization(self,
                                           collection_name: str,
...
class AdaptiveIndexManager:
    ...
    def __init__(self, 
                 indexing_manager: IndexingStrategyManager,
        ...
        try:
            ...
            if hours_since_last < self.optimization_interval_hours:
                return {
                    ...
                    'next_optimization_in_hours': self.optimization_interval_hours - hours_since_last
                }
            ...
            high_impact_recommendations = [
                rec for rec in analysis_results['index_recommendations']
            ...
            optimization_results = {
                ...
                'analysis_summary': {
                    'total_recommendations': len(analysis_results['index_recommendations']),
                    'high_impact_recommendations': len(high_impact_recommendations),
                    'queries_analyzed': analysis_results['analysis_period']['total_queries']
                }
            }
...
class QueryOptimizer:
    ...
    def __init__(self, qdrant_store: QdrantVectorStore):
        ...
        self.adaptive_manager = AdaptiveIndexManager(self.indexing_manager, qdrant_store)
        self.logger = logger
    ...
    async def get_optimization_dashboard(self) -> Dict[str, Any]:
        ...
        try:
            ...
            dashboard = {
                ...
                'current_recommendations': analysis_results['index_recommendations'][:10],
    ...
    async def force_optimization(self) -> Dict[str, Any]:
        ...
        try:
            self.adaptive_manager.last_optimization = datetime.min  # Reset to force optimization
            return await self.adaptive_manager.run_adaptive_optimization()
        except Exception as e:
            self.logger.error(f"Forced optimization failed: {e}")
            return {'status': 'failed', 'error': str(e)}
    
    async def apply_specific_recommendation(self, recommendation_data: Dict[str, Any]) -> bool:
        ...
        try:
            recommendation = IndexRecommendation(
                ...
                estimated_improvement_ms=recommendation_data.get('estimated_improvement_ms', 0),
                reasoning=recommendation_data.get('reasoning', 'Manual application')
            )
            
            return await self.indexing_manager.apply_index_recommendation(recommendation)
