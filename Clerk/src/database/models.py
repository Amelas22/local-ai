...
class LawFirm(Base):
    ...
    # Relationships
    users = relationship(
        "User", back_populates="law_firm", cascade="all, delete-orphan"
    )
    cases = relationship(
        "Case", back_populates="law_firm", cascade="all, delete-orphan"
    )
...
class User(Base):
    ...
    law_firm_id = Column(
        String(36), ForeignKey("law_firms.id", ondelete="CASCADE"), nullable=False
    )
    is_active = Column(Boolean, default=True)
...
class Case(Base):
    ...
    permissions = relationship(
        "UserCasePermission", back_populates="case", cascade="all, delete-orphan"
    )
...
class UserCasePermission(Base):
    ...
    granter = relationship(
        "User", foreign_keys=[granted_by], back_populates="granted_permissions"
    )
