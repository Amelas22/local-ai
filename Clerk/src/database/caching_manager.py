...
from ..models.normalized_document_models import DocumentCore, DocumentMetadata, ChunkMetadata
...
class MaterializedViewManager:
    ...
    async def create_materialized_view(self,
                                     view_name: str,
                                     query_config: Dict[str, Any],
                                     refresh_interval_hours: int = 24) -> bool:
    ...
    async def refresh_materialized_view(self, view_name: str) -> bool:
        ...
        try:
            ...
            # Schedule next refresh
            next_refresh = datetime.now() + timedelta(hours=view['refresh_interval_hours'])
            self.view_refresh_schedule[view_name] = next_refresh
    ...
    async def _execute_aggregation_query(self, query_config: Dict[str, Any]) -> Dict[str, Any]:
...
class CachingManager:
    ...
    def __init__(self,
        ...
        self.invalidation_rules: Dict[str, Set[str]] = {}  # entity_type -> cache_patterns
    ...
    async def get(self, 
                ...
                fallback_func: Optional[callable] = None) -> Tuple[Optional[Any], CacheLevel]:
        """
    ...
    async def put(self,
        ...
        try:
            ...
            # Calculate size
            size_bytes = len(pickle.dumps(value)) if self.config.enable_compression else len(str(value))
    ...
    async def invalidate(self, 
                       cache_key: Union[str, CacheKey, List[str]],
    ...
    async def invalidate_by_pattern(self, pattern: str) -> int:
        ...
        try:
            ...
            self.logger.info(f"Cleared {total_cleared} cache entries matching pattern: {pattern}")
    ...
    def _is_materialized_view_candidate(self, key: str) -> bool:
        ...
        return any(pattern in key for pattern in ['aggregation', 'stats', 'summary', 'count'])
    ...
    def _record_hit(self, cache_level: CacheLevel, start_time: float):
        ...
        self.stats.avg_response_time_ms = (
            (self.stats.avg_response_time_ms * (self.stats.hits - 1) + response_time) / self.stats.hits
        )
