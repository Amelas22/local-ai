...
class FloridaStatutesLoader:
    ...
    def _compile_statute_patterns(self) -> Dict[str, re.Pattern]:
        ...
        return {
            ...
            'effective_date': re.compile(
                r'Effective\s+Date:\s*([A-Za-z]+ \d+, \d{4})',
                re.IGNORECASE
            )
        }
    ...
    def _define_topic_mappings(self) -> Dict[str, List[str]]:
        ...
        return {
            'negligence': ['negligence', 'fault', 'liability', 'duty of care', 'breach'],
            'motor_vehicle': ['vehicle', 'driver', 'traffic', 'motor carrier', 'commercial'],
            'personal_injury': ['injury', 'damages', 'compensation', 'harm', 'recovery'],
            'civil_procedure': ['procedure', 'pleading', 'motion', 'discovery', 'trial'],
            'evidence': ['evidence', 'admissible', 'testimony', 'witness', 'exhibit'],
            'insurance': ['insurance', 'coverage', 'policy', 'insurer', 'claim'],
            'premises_liability': ['premises', 'property', 'owner', 'invitee', 'trespasser'],
            'product_liability': ['product', 'defect', 'manufacturer', 'design', 'warning'],
            'medical_malpractice': ['medical', 'malpractice', 'physician', 'hospital', 'standard of care'],
    ...
    async def load_chapter(
        ...
        logger.info(f"Loading Florida Statutes Chapter {chapter_number}: {chapter_title}")
    ...
    def _parse_chapter_sections(
        ...
        for i in range(1, len(section_splits), 3):
            if i + 2 < len(section_splits):
                ...
                sections.append((section_num, section_title.strip(), section_content.strip()))
    ...
    def _extract_topics(self, text: str) -> List[str]:
        ...
        legal_concepts = [
            'comparative_fault', 'statute_of_limitations', 'sovereign_immunity',
            'res_judicata', 'collateral_estoppel', 'proximate_cause',
            'respondeat_superior', 'vicarious_liability', 'punitive_damages'
        ]
    ...
    async def get_statute_by_number(self, statute_number: str) -> Optional[SharedKnowledgeEntry]:
    ...
    async def load_common_statutes(self):
        ...
        for statute in common_statutes:
            await self.load_statute(
                ...
                chapter="Chapter 768 - Negligence" if statute["number"].startswith("768") else "Chapter 316 - Traffic",
