...
def validate_case_access(case_name: str) -> bool:
    ...
    if normalized_case not in ALLOWED_CASE_NAMES:
        logger.warning(
            f"Access denied to case: '{normalized_case}'. Allowed cases: {ALLOWED_CASE_NAMES}"
        )
...
def validate_embedding_vector(
    vector: List[float], expected_dimension: int = 1536
) -> bool:
    """
    ...
    for i, value in enumerate(vector):
        ...
        if not (-1.0 <= value <= 1.0):
            logger.warning(
                f"Vector element at index {i} is outside normal range: {value}"
            )
