...
class TokenUsage:
    ...
    def calculate_cost(self, pricing: Dict[str, float]) -> float:
        ...
        if self.operation == "embedding":
            ...
            input_cost = (self.prompt_tokens / 1000) * model_pricing.get(
                "input_per_1k", 0
            )
            output_cost = (self.completion_tokens / 1000) * model_pricing.get(
                "output_per_1k", 0
            )
...
class DocumentCost:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "cost": {
                "embedding_cost": sum(
                    u.calculate_cost(OPENAI_PRICING) for u in self.embedding_calls
                ),
                "context_cost": sum(
                    u.calculate_cost(OPENAI_PRICING) for u in self.context_calls
                ),
...
class CostTracker:
    ...
    def start_document(
        self, document_name: str, document_id: str, case_name: str
    ) -> DocumentCost:
        ...
        doc_cost = DocumentCost(
            document_name=document_name, document_id=document_id, case_name=case_name
        )
        self.documents[document_id] = doc_cost
        return doc_cost

    def track_embedding_usage(
        self, document_id: str, tokens: int, model: str = "text-embedding-3-small"
    ):
    ...
    def finish_document(
        self, document_id: str, chunks_processed: int, processing_time: float
    ):
    ...
    def get_session_report(self) -> Dict[str, Any]:
        ...
        successful_documents = sum(
            1 for d in self.documents.values() if d.chunks_processed > 0
        )
        ...
        report = {
            ...
            "session_duration": (
                datetime.utcnow() - self.current_session_start
            ).total_seconds(),
            "summary": {
                "total_documents": total_documents,
                "successful_documents": successful_documents,
                "total_chunks": sum(
                    d.chunks_processed for d in self.documents.values()
                ),
                "total_api_calls": sum(
            ...
            "costs": {
                ...
                "average_per_document": round(
                    (total_embedding_cost + total_context_cost)
    ...
    def print_summary(self):
        ...
        if report["costs_by_case"]:
            ...
            for case, data in report["costs_by_case"].items():
                print(
                    f"  {case}: ${data['cost']:.4f} ({data['documents']} docs, {data['tokens']:,} tokens)"
                )
        ...
        if report["top_5_expensive_documents"]:
            ...
            for i, doc in enumerate(report["top_5_expensive_documents"], 1):
                print(
                    f"  {i}. {doc['document_name']}: ${doc['cost']['total']:.4f} ({doc['tokens']['total']:,} tokens)"
                )
