...
MAX_FILE_SIZE_MB = int(os.getenv("DISCOVERY_MAX_FILE_SIZE_MB", DEFAULT_MAX_FILE_SIZE_MB))
...
def validate_pdf_content(content: bytes, filename: Optional[str] = None) -> Tuple[bool, str]:
    """
    ...
    if len(content) > MAX_FILE_SIZE_BYTES:
        return False, f"The {file_desc} exceeds size limit ({size_mb:.1f}MB > {MAX_FILE_SIZE_MB}MB)"
...
def validate_base64_pdf(base64_data: str, filename: Optional[str] = None) -> Tuple[bool, str]:
    """
