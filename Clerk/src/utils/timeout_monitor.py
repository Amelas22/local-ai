...
class ProgressTracker:
    ...
    def next_step(self, step_name: str, description: str = "", metadata: Optional[Dict[str, Any]] = None):
        """Move to next step"""
    ...
    def finish(self):
        ...
        if sorted_steps:
            logger.info(f"[PROGRESS] Slowest steps:")
            for step in sorted_steps[:3]:
                logger.info(
                    f"  - {step.step_name}: {step.duration:.1f}s"
                )
...
def timeout_monitored(warning_threshold: float = 60.0, critical_threshold: float = 120.0):
    """
...
class OperationTimer:
    ...
    def __enter__(self):
        ...
        logger.log(
            ...
            f"[TIMER] Starting {self.operation_name}"
        )
        return self
