            ...
            case_data.append(
                {
                    ...
                    "Average Cost per Doc ($)": data["cost"]
                    / max(data["documents"], 1),
                }
            )
    ...
    def _create_api_usage_sheet(self, writer):
        ...
        for doc in self.report["all_documents"]:
            ...
            if embed_model not in model_usage:
                model_usage[embed_model] = {"calls": 0, "tokens": 0, "cost": 0.0}
                ...
                pricing_data.append(
                    {
                        ...
                        "Price per 1M Tokens ($)": pricing.get("input_per_1k", 0)
                        * 1000,
                    }
                )
                pricing_data.append(
                    {
                        ...
                        "Price per 1M Tokens ($)": pricing.get("output_per_1k", 0)
                        * 1000,
                    }
                )
...
def generate_cost_comparison_report(
    reports: Dict[str, Dict[str, Any]], output_path: Optional[str] = None
) -> str:
