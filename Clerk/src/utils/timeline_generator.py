...
class TimelineGenerator:
    ...
    async def generate_timeline(
        ...
        timeline = FactTimeline(
            case_name=self.case_name, timeline_events=[], date_ranges=[], key_dates={}
        )
        ...
        for point in results[0]:
            ...
            fact = CaseFact(
                ...
                extraction_timestamp=datetime.fromisoformat(
                    point.payload["extraction_timestamp"]
                ),
                ...
                date_references=[
                    DateReference(
                        ...
                        start_date=datetime.fromisoformat(
                            point.payload["primary_date"]
                        ),
                    )
                ]
                if "primary_date" in point.payload
                else [],
            )
            facts.append(fact)
    ...
    def _identify_key_dates(self, timeline: FactTimeline) -> Dict[str, datetime]:
        ...
        for date, fact in timeline.timeline_events:
            ...
            if any(
                ...
                for word in ["incident", "accident", "crash", "injury"]
            ):
    ...
    async def _store_timeline(self, timeline: FactTimeline):
        ...
        if points:
            self.vector_store.client.upsert(
                collection_name=self.timeline_collection, points=points
            )
            logger.info(
                f"Stored {len(points)} timeline events in {self.timeline_collection}"
            )

    def generate_narrative_timeline(
        self, timeline: FactTimeline, format: str = "markdown"
    ) -> str:
    ...
    def _generate_markdown_timeline(self, timeline: FactTimeline) -> str:
        ...
        lines.extend(["## Chronological Events", ""])
    ...
    def _generate_text_timeline(self, timeline: FactTimeline) -> str:
        ...
        lines = [f"CASE TIMELINE: {self.case_name}", "=" * 50, ""]
    ...
    def _generate_json_timeline(self, timeline: FactTimeline) -> str:
        ...
        data = {
            "case_name": self.case_name,
            "timeline_period": {
                "start": timeline.key_dates.get(
                    "case_start", datetime.now()
                ).isoformat(),
                "end": timeline.key_dates.get("case_end", datetime.now()).isoformat(),
            },
            "key_dates": {
                name: date.isoformat() for name, date in timeline.key_dates.items()
            },
            "events": [
                {
                    "date": date.isoformat(),
    ...
    async def find_events_near_date(
        self, target_date: datetime, days_before: int = 30, days_after: int = 30
