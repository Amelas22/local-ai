...
def sanitize_log_data(data: Any) -> Any:
    ...
    if isinstance(data, dict):
        ...
        if len(data) > 8 and any(
            pattern in data.lower() for pattern in ["password", "secret", "key"]
        ):
            return "***REDACTED***"
        return data
...
def setup_logging(
    ...
    file_handler = logging.handlers.RotatingFileHandler(
        log_dir / f"{app_name}.log", maxBytes=MAX_LOG_SIZE, backupCount=BACKUP_COUNT
    )
    ...
    security_handler = logging.handlers.RotatingFileHandler(
        log_dir / "security.log", maxBytes=MAX_LOG_SIZE, backupCount=BACKUP_COUNT
    )
    ...
    performance_handler = logging.handlers.RotatingFileHandler(
        log_dir / "performance.log", maxBytes=MAX_LOG_SIZE, backupCount=BACKUP_COUNT
    )
...
class QueryLogger:
    ...
    def __init__(
        self, logger: logging.Logger, query_id: str, user_id: str, case_name: str
    ):
        self.logger = logger
...
def log_performance_metric(
    metric_name: str, value: float, unit: str, context: Dict[str, Any] = None
):
    """
