...
class FactualFinding(BaseModel):
    ...
    supporting_evidence: List[str] = Field(default_factory=list, description="Additional supporting evidence")
...
class ResearchMemo(BaseModel):
    ...
    legal_precedents: List[LegalCitation] = Field(default_factory=list, description="Relevant legal precedents")
    factual_findings: List[FactualFinding] = Field(default_factory=list, description="Key factual findings")
    ...
    research_gaps: List[str] = Field(default_factory=list, description="Areas needing additional research")
    confidence_level: float = Field(..., ge=0.0, le=1.0, description="Confidence in research findings")
...
class TimelineEvent(BaseModel):
    ...
    key_parties: List[str] = Field(default_factory=list, description="Key parties involved")

class CaseTimeline(BaseModel):
    """Complete case timeline"""
    case_name: str = Field(..., description="Case name")
    timeline_events: List[TimelineEvent] = Field(..., description="Chronological events")
    critical_dates: List[str] = Field(default_factory=list, description="Critical dates for litigation")
    statute_of_limitations: Optional[str] = Field(None, description="Relevant statute of limitations")
...
async def search_legal_precedents(
    ...
    try:
        ...
        query_embedding, _ = ctx.deps.embedding_generator.generate_embedding(precedent_query)
...
async def investigate_factual_timeline(
    ...
    try:
        ...
        timeline_results = sorted(unique_results.values(), key=lambda x: x.score, reverse=True)[:15]
...
async def analyze_expert_testimony(
    ...
    try:
        ...
        query_embedding, _ = ctx.deps.embedding_generator.generate_embedding(expert_query)
...
async def compare_similar_cases(
    ...
    try:
        ...
        query_embedding, _ = ctx.deps.embedding_generator.generate_embedding(similarity_query)
        ...
        if comparison_factors:
            ...
            for factor in comparison_factors:
                ...
                factor_embedding, _ = ctx.deps.embedding_generator.generate_embedding(factor_query)
        ...
        if not similar_results:
            return f"No similar cases found for comparison based on: {case_similarities}"
