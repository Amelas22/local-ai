...
from src.models.fact_models import FactCategory, CaseIsolationConfig
...
class EvidenceMapper:
    ...
    def __init__(self, case_name: str):
        ...
        self.isolation_config = CaseIsolationConfig(
            case_name=case_name, enable_shared_knowledge=True, audit_access=True
        )
    ...
    async def map_evidence_to_outline(
        ...
        for section in outline_sections:
            ...
            mapping = EvidenceMapping(
                ...
                total_relevance_score=sum(e.relevance_score for e in top_evidence)
                / len(top_evidence)
                if top_evidence
                else 0,
    ...
    async def _find_evidence_for_argument(
        self, section: ArgumentSection, limit: int
    ) -> List[EvidenceItem]:
        ...
        facts = await self.fact_extractor.search_facts(
            query=search_query, limit=limit // 3
        )
            ...
            evidence_items.append(
                EvidenceItem(
                    ...
                    metadata={
                        ...
                        "status": exhibit.authenticity_status,
                    },
                )
            )
    ...
    async def _score_evidence_relevance(
        self, section: ArgumentSection, evidence_items: List[EvidenceItem]
    ) -> List[EvidenceItem]:
...
Format: SCORE: X.X | REASON: explanation"""
        ...
        for item in evidence_items:
            ...
            if item.evidence_type == "fact":
                if item.metadata.get("category") == FactCategory.TIMELINE:
                    ...
                    suggestion = (
                        f"Cite to support damages claim: {item.citation_format}"
                    )
                else:
    ...
    def _summarize_evidence(self, evidence_items: List[EvidenceItem]) -> str:
        ...
        avg_relevance = sum(e.relevance_score for e in evidence_items) / len(
            evidence_items
        )
    ...
    async def create_evidence_report(self, mappings: List[EvidenceMapping]) -> str:
        ...
        for mapping in mappings:
            ...
            for evidence_type, items in by_type.items():
                ...
                for item in items:
                    usage = mapping.suggested_usage.get(
                        item.id, "Use as supporting evidence"
                    )
                    lines.append(
                        f"- {item.citation_format} ({item.relevance_score:.0%})"
                    )
