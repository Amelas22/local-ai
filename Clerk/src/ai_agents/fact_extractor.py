...
from src.models.fact_models import (
    ...
    CaseFactCollection, FactExtractionRequest
)
...
class FactExtractor:
    ...
    def _ensure_collections_exist(self):
        """Create case-specific collections if they don't exist"""
        ...
        for collection_name, vector_config in collections_to_create:
            try:
    ...
    def _compile_patterns(self) -> Dict[str, re.Pattern]:
        ...
        return {
            ...
            'date_written': re.compile(r'\b(January|February|March|April|May|June|July|August|September|October|November|December)\s+(\d{1,2}),?\s+(\d{4})\b', re.I),
            'date_iso': re.compile(r'\b(\d{4})-(\d{2})-(\d{2})\b'),
            
            # Deposition citation patterns
            'deposition': re.compile(r'(\w+)\s+Dep\.\s*(?:at\s*)?(\d+):(\d+)(?:-(\d+))?(?::(\d+))?'),
            'deposition_long': re.compile(r'Deposition\s+of\s+([^,]+),\s*(?:taken\s+on\s+)?([^,]+),\s*(?:at\s*)?(?:pp?\.\s*)?(\d+)(?:-(\d+))?'),
            
            # Legal citation patterns
            'case_citation': re.compile(r'(\d+)\s+(\w+\.?\s*\d*[dst]?)\s+(\d+)(?:\s*\(([^)]+)\))?'),
            ...
            # Vehicle patterns
            'vehicle': re.compile(r'\b\d{4}\s+\w+\s+\w+\b|\b(?:car|truck|vehicle|trailer|semi)\b', re.I),
            
            # Medical patterns
            'medical': re.compile(r'\b(?:diagnosis|injury|treatment|surgery|fracture|trauma|condition)\b', re.I)
        }
    ...
    async def extract_facts_from_document(
        ...
        logger.info(f"Extracting facts from document {document_id} for case {self.case_name}")
    ...
    async def _extract_facts_from_chunk(
        ...
        try:
            ...
            response = self.openai_client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "You are a legal fact extractor. Extract key factual statements from legal text."},
    ...
    def _extract_dates(self, text: str) -> List[DateReference]:
        ...
        for sentence in sentences:
            if any(month in sentence.lower() for month in ['january', 'february', 'march', 'april', 'may', 'june', 
    ...
    def _extract_entities(self, text: str) -> Dict[EntityType, List[str]]:
        ...
        party_pattern = re.compile(r'^(Plaintiff|Defendant|Petitioner|Respondent|Appellant|Appellee|Claimant)\s+(.+)$', re.IGNORECASE)
        ...
        for pattern in party_patterns:
            for match in re.finditer(pattern, text):
                if match.lastindex == 2:
                    # Pattern 1: Role Name
                    name = match.group(2) if match.group(1) in ['Plaintiff', 'Defendant', 'Petitioner', 'Respondent', 'Appellant', 'Appellee', 'Claimant'] else match.group(1)
                else:
                    # Pattern 2: Name Role
                    name = match.group(1)
                
                # Only add if it looks like a proper name (not all caps like "PLAINTIFF")
                if name and not name.isupper() and name not in entities[EntityType.PERSON]:
    ...
    async def _store_facts(self, collection: CaseFactCollection):
        ...
        for fact in collection.facts:
            ...
            embedding, _ = await self.embedding_generator.generate_embedding_async(fact.content)
    ...
    async def search_facts(
        ...
        for result in results:
            ...
            fact = CaseFact(
                ...
                extraction_timestamp=datetime.fromisoformat(result.payload["extraction_timestamp"]),
