...
class ResearchRequest(BaseModel):
    ...
    research_context: str = Field(
        ..., description="Context about what type of research is needed"
    )
    section_type: str = Field(
        default="argument", description="Type of section being drafted"
    )
...
class RAGResearchAgent:
    ...
    def __init__(self):
        ...
        logger.info(
            f"RAGResearchAgent initialized with model: {settings.ai.rag_agent_model}"
        )
...
Make decisions quickly and confidently based on the question type.""",
    ...
    async def research_questions(self, request: ResearchRequest) -> ResearchResponse:
        """
        Main research method that processes questions and returns structured results
        """
        logger.info(
            f"[RAG_RESEARCH] Starting research for {len(request.questions)} questions"
        )
        ...
        try:
            ...
            logger.info(
                f"[RAG_DEBUG] All available collections at start: {collection_names}"
            )
        except Exception as e:
        ...
        try:
            ...
            logger.info(
                f"[RAG_RESEARCH] Generated {len(research_queries)} optimized queries"
            )

            # Step 2: Execute searches
            search_results = await self._execute_searches(
                research_queries, request.database_name
            )
            logger.info(
                f"[RAG_RESEARCH] Executed searches, got {len(search_results)} results"
            )
            ...
            logger.info(
                f"[RAG_RESEARCH] Research completed: {response.total_results} total results"
            )

        except Exception as e:
            logger.error(
                f"[RAG_RESEARCH] Error in research process: {str(e)}", exc_info=True
            )
    ...
    async def _process_questions(self, questions: List[str]) -> List[ResearchQuery]:
        ...
        for i, question in enumerate(questions[:10]):  # Limit to 10 questions
            try:
                logger.info(
                    f"[RAG_RESEARCH] Processing question {i + 1}: {question[:80]}..."
                )
                ...
                logger.error(
                    f"[RAG_RESEARCH] Error processing question '{question[:50]}': {str(e)}"
                )
                continue
    ...
    async def _optimize_query(self, question: str) -> List[str]:
        ...
        try:
            result = await asyncio.wait_for(
                self.query_optimizer.run(
                    f"Optimize this question for database search: {question}"
                ),
                timeout=10.0,
            )
            ...
            logger.info(
                f"[RAG_RESEARCH] Optimized '{question[:30]}...' into {len(queries)} queries"
            )
    ...
    async def _select_database(self, question: str) -> DatabaseType:
        ...
        try:
            result = await asyncio.wait_for(
                self.database_selector.run(question), timeout=5.0
            )
    ...
    def _classify_query_type(self, question: str) -> QueryType:
        ...
        if any(
            ...
            for kw in ["precedent", "case", "court", "ruling", "law"]
        ):
            return QueryType.LEGAL_PRECEDENTS
        elif any(
            kw in question_lower
            for kw in ["expert", "opinion", "analysis", "reconstruction"]
        ):
            return QueryType.EXPERT_EVIDENCE
        elif any(
            kw in question_lower
            for kw in ["regulation", "fmcsr", "dot", "compliance", "violation"]
        ):
            return QueryType.REGULATORY_COMPLIANCE
        elif any(
            kw in question_lower
            for kw in ["motion", "filed", "ruling", "discovery", "procedural"]
        ):
            return QueryType.PROCEDURAL_HISTORY
        elif any(
            kw in question_lower
            for kw in ["argument", "strategy", "successful", "effective"]
        ):
    ...
    async def _execute_searches(
        self, research_queries: List[ResearchQuery], case_database_name: str
    ) -> List[ResearchResult]:
        ...
        for i, query in enumerate(research_queries):
            try:
                logger.info(
                    f"[RAG_RESEARCH] Executing query {i + 1}/{len(research_queries)}: {query.query_text[:50]}..."
                )
                ...
                databases_to_search = self._get_databases_to_search(
                    query, case_database_name
                )
                ...
                for db_name, db_type in databases_to_search:
                    try:
                        ...
                        logger.info(
                            f"[RAG_DEBUG] Attempting search on database: '{db_name}' (type: {db_type.value})"
                        )
                        ...
                        try:
                            ...
                            logger.info(
                                f"[RAG_DEBUG] Available collections: {collection_names}"
                            )

                            if db_name not in collection_names:
                                logger.warning(
                                    f"[RAG_DEBUG] Collection '{db_name}' not found! Available: {collection_names}"
                                )
                                continue
                            ...
                            logger.info(
                                f"[RAG_DEBUG] Collection '{db_name}' has {collection_info.points_count} points"
                            )

                            if collection_info.points_count == 0:
                                logger.warning(
                                    f"[RAG_DEBUG] Collection '{db_name}' is empty!"
                                )
                                continue

                        except Exception as check_error:
                            logger.error(
                                f"[RAG_DEBUG] Error checking collection '{db_name}': {check_error}"
                            )
                            continue
                        ...
                        logger.info(
                            f"[RAG_DEBUG] Search returned {len(results)} raw results from '{db_name}'"
                        )

                        # Convert to ResearchResult objects
                        for j, result in enumerate(results):
                            logger.info(
                                f"[RAG_DEBUG] Result {j + 1}: score={result.score:.4f}, content_length={len(result.content)}"
                            )

                            # No score filtering - hybrid search already ranks results
                            research_result = ResearchResult(
                                content=result.content[:1000],  # Limit content length
                                source_document=result.metadata.get(
                            ...
                            logger.info(
                                f"[RAG_RESEARCH] Added result (score: {result.score:.3f}) from {db_name}"
                            )

                    except Exception as e:
                        logger.error(
                            f"[RAG_RESEARCH] Error searching {db_name}: {str(e)}",
                            exc_info=True,
                        )
                        continue
                ...
                logger.error(
                    f"[RAG_RESEARCH] Error executing query '{query.query_text[:30]}': {str(e)}"
                )
                continue

        logger.info(
            f"[RAG_RESEARCH] Collected {len(search_results)} total search results"
        )
        return search_results

    def _get_databases_to_search(
        self, query: ResearchQuery, case_database_name: str
    ...
    def _categorize_results(
        self, search_results: List[ResearchResult], response: ResearchResponse
    ) -> ResearchResponse:
        ...
        for category in [
            response.legal_precedents,
            response.case_facts,
            response.expert_evidence,
            response.regulatory_compliance,
            response.procedural_history,
            response.argument_strategies,
        ]:
            category.sort(
                key=lambda x: (
                    self._get_evidence_priority_score(
                        x
                    ),  # Priority for specific evidence
    ...
    def _get_evidence_priority_score(self, result_dict: Dict[str, Any]) -> float:
        ...
        if result_dict.get("formatted_citation") and ":" in str(
            result_dict["formatted_citation"]
        ):
            base_score += 0.1
