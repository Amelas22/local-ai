...
from src.models.source_document_models import (
    DocumentType, DocumentRelevance, EvidenceSearchQuery,
    EvidenceSearchResult, SourceDocument
)
...
"""
    ...
    async def _create_evidence_strategy(
        self,
        argument: str,
        suggestions: List[ExhibitSuggestion],
        motion_type: str
    ) -> str:
        """Create a strategy for using the evidence together"""
        
        if not suggestions:
            return "No supporting evidence found. Consider gathering additional discovery."
        
        doc_summary = "\n".join([
            f"- {s.suggested_exhibit_label}: {s.purpose}"
...
"""
    ...
    async def suggest_exhibits_for_motion(
        self,
        motion_sections: List[Dict[str, str]],
        motion_type: str
