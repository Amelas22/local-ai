...
class CitationFormatter:
    ...
    def _compile_patterns(self) -> Dict[str, re.Pattern]:
        ...
        return {
            ...
            "deposition_page_line": re.compile(r"(\d+:\d+(?:[-]\d+)?)", re.IGNORECASE),
            # Exhibit patterns
            "exhibit": re.compile(
                r"(?:Exhibit|Ex\.|Exh\.)\s*([A-Z0-9]+(?:[-][A-Z0-9]+)?)", re.IGNORECASE
            ),
            ...
            "regulation": re.compile(
                r"(\d+)\s+C\.F\.R\.\s*(?:§|section)*\s*(\d+\.\d+)", re.IGNORECASE
            ),
            # FMCSR patterns
            "fmcsr": re.compile(
                r"FMCSR\s+(\d+\.\d+(?:\([a-z]\)(?:\(\d+\))?)?)", re.IGNORECASE
            ),
        }
    ...
    def format_search_result_as_citation(
        self, search_result: Dict[str, Any], citation_hint: Optional[str] = None
    ) -> Optional[Citation]:
    ...
    def _format_internal_doc_citation(
        ...
        if "email" in doc_type:
            ...
            sender = (
                sender_match.group(1).strip() if sender_match else "Company Official"
            )
    ...
    def enhance_text_with_citations(
        self, text: str, search_results: List[Dict[str, Any]]
    ) -> str:
    ...
    def create_citation_index(
        self, drafted_sections: List[Any]
