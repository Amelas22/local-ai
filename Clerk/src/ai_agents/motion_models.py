...
class OutlineSectionInput(BaseModel):
    ...
    points: List[str] = Field(
        default_factory=list, description="Content points to cover"
    )
    authorities: List[str] = Field(
        default_factory=list, description="Legal authorities to cite"
    )
    subsections: Optional[List["OutlineSectionInput"]] = Field(
...
class DraftingRequest(BaseModel):
    ...
    target_length: str = Field(
        "MEDIUM", description="Target length: SHORT, MEDIUM, or LONG"
    )
    enable_case_research: bool = Field(
        True, description="Enable case document retrieval"
    )
...
class MotionDraftResponse(BaseModel):
    ...
    coherence_score: float = Field(
        0.0, ge=0.0, le=1.0, description="Document coherence score"
    )
    review_notes: List[str] = Field(default_factory=list, description="Review feedback")
    export_links: Dict[str, str] = Field(
        default_factory=dict, description="Links to exported formats"
    )
...
def estimate_pages_from_outline(outline: MotionOutlineInput) -> int:
    ...
    subsections_count = sum(
        len(s.subsections) if s.subsections else 0 for s in outline.sections
    )
    ...
    estimated = (sections_count * pages_per_section) + (
        subsections_count * pages_per_subsection
    )
