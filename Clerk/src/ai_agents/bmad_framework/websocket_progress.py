...
class ProgressTracker:
    ...
    async def emit_start(self, message: str = "", metadata: Optional[Dict[str, Any]] = None) -> None:
        """
    ...
    async def update_total_steps(self, total_steps: int) -> None:
        ...
        await self.emit_progress(
            message=f"Updated total steps to {total_steps}"
        )
...
class BatchProgressTracker:
    ...
    async def start_task(self, task_name: str, total_steps: Optional[int] = None) -> ProgressTracker:
        """
    ...
    async def _emit_workflow_progress(self) -> None:
        ...
        completed_tasks = len([t for t in self.trackers.values() 
        ...
        event_data = {
            ...
            "current_task": self.tasks[self.current_task_index] if self.current_task_index >= 0 else None,
        ...
        if WEBSOCKET_AVAILABLE and sio:
            try:
                await sio.emit(
                    "agent:workflow_progress",
                    event_data,
                    room=f"case_{self.case_id}"
                )
            except Exception as e:
