...
from src.models.case_models import Case, CaseContext
...
class ClerkIntegration:
    ...
    async def process_pdf_document(
        ...
        try:
            ...
            if progress_tracker:
                await progress_tracker.emit_progress(message=f"Processing PDF: {file_path.name}")
    ...
    def create_agent_executor_with_integration(
        ...
        executor.register_command_handler(
            ...
            lambda ctx: self._handle_search_command(ctx)
        )
        
        executor.register_command_handler(
            "parse_rtp",
            lambda ctx: self._handle_parse_rtp_command(ctx)
        )
