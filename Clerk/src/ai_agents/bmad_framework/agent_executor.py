...
class AgentExecutor:
    ...
    async def execute_command(
        ...
        security_context.log_operation(
            "command_execution",
            {"command": command, "case": case_name}
        )
    ...
    async def _execute_task_command(self, context: ExecutionContext) -> Any:
        ...
        if not task_name:
            raise TaskExecutionError(
                context.command,
                "No task mapping found for command"
            )
    ...
    def _find_task_for_command(self, context: ExecutionContext) -> Optional[str]:
        ...
        for task in context.agent_def.tasks:
            ...
            if task_base == context.command or task_base.startswith(f"{context.command}-"):
                return task
    ...
    async def _load_task(self, task_name: str) -> str:
        ...
        try:
            ...
            raise TaskExecutionError(
                task_name,
                f"Failed to read task file: {str(e)}"
            )
    ...
    def _parse_task_content(self, content: str) -> Dict[str, Any]:
        ...
        for line in lines:
            ...
            elif task_def["current_section"] == "execution" and line.startswith(("1.", "2.", "3.", "-")):
                task_def["execution_steps"].append(line)
            elif task_def["current_section"] == "elicitation" and "true" in line.lower():
    ...
    async def _execute_task_steps(
        ...
        for i, step in enumerate(task_def["execution_steps"]):
            ...
            if task_def["elicit"] and context.elicitation_handler:
                ...
                results.append({"step": i + 1, "description": step, "response": response})
            else:
                # Simulate step execution
                results.append({"step": i + 1, "description": step, "status": "completed"})
    ...
    async def _emit_websocket_event(
        ...
        if WEBSOCKET_AVAILABLE and sio and context.websocket_channel:
            try:
                ...
                if context.websocket_channel != f"case_{context.security_context.case_id}":
                    await sio.emit(event_type, event_data, room=context.websocket_channel)
