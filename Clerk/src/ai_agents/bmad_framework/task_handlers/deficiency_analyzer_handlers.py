...
class DeficiencyAnalyzerHandlers:
    ...
    async def handle_analyze_rtp(self, context: ExecutionContext) -> Dict[str, Any]:
        ...
        try:
            ...
            if context.websocket_channel:
                await self._emit_progress(context, "Parsing RTP document...", 10)
            ...
            if context.websocket_channel:
                await self._emit_progress(
                    context, f"Found {len(rtp_requests)} RTP requests", 50
                )
            ...
            # Complete
            if context.websocket_channel:
                await self._emit_progress(context, "RTP analysis complete", 100)
    ...
    async def handle_search_production(
        ...
        try:
            ...
            if context.websocket_channel:
                await self._emit_progress(
                    context, "Searching production documents...", 20
                )
            ...
            (
                embeddings,
                _,
            ) = await self.embedding_generator.generate_embeddings_batch_async([query])
            ...
            if context.websocket_channel:
                await self._emit_progress(
                    context, f"Found {len(search_results)} relevant documents", 80
                )
            ...
            # Complete
            if context.websocket_channel:
                await self._emit_progress(context, "Search complete", 100)
    ...
    async def handle_categorize_compliance(
        self, context: ExecutionContext
    ) -> Dict[str, Any]:
        """
        ...
        if not request_number or not request_text:
            raise ValidationError(
                "parameters", "request_number and request_text are required"
            )
        ...
        try:
            ...
            if context.websocket_channel:
                await self._emit_progress(
                    context, f"Categorizing request {request_number}...", 30
                )
            ...
            classification = self._determine_classification(
                search_results, oc_response_text
            )

            # Calculate confidence score
            confidence_score = self._calculate_confidence(
                classification, search_results, oc_response_text
            )
            ...
            recommendation = self._generate_recommendation(
                classification, confidence_score, evidence_summary
            )

            # Emit progress
            if context.websocket_channel:
                await self._emit_progress(
                    context, f"Categorized as {classification}", 90
                )
            ...
            # Complete
            if context.websocket_channel:
                await self._emit_progress(context, "Categorization complete", 100)
    ...
    async def handle_full_analysis(self, context: ExecutionContext) -> Dict[str, Any]:
        ...
        if not production_id or not rtp_document_id:
            raise ValidationError(
                "parameters", "production_id and rtp_document_id are required"
            )
        ...
        try:
            ...
            asyncio.create_task(
                self._process_full_analysis(context, report, processing_id)
            )
    ...
    def _determine_classification(
        self, search_results: List[Dict], oc_response: str
    ) -> str:
    ...
    def _calculate_confidence(
        self, classification: str, search_results: List[Dict], oc_response: str
    ) -> float:
        ...
        if search_results:
            avg_score = sum(r.get("relevance_score", 0) for r in search_results) / len(
                search_results
            )
    ...
    def _generate_recommendation(
        self, classification: str, confidence: float, evidence_summary: str
    ) -> str:
    ...
    async def _emit_progress(
        self, context: ExecutionContext, message: str, percentage: int
    ) -> None:
    ...
    async def _process_full_analysis(
        self, context: ExecutionContext, report: DeficiencyReport, processing_id: str
    ) -> None:
        """
        ...
        await self.deficiency_service.update_analysis_status(
            report_id=str(report.id), status="completed"
        )
