...
import pytest
from src.ai_agents.bmad_framework.api_mapper import (
    APIMapper,
    APIMapping,
    HTTPMethod
)
...
class TestAPIMapping:
    ...
    def test_format_endpoint(self):
        ...
        formatted = mapping.format_endpoint(
            agent_id="test-agent",
            resource_id="123"
        )
...
class TestAPIMapper:
    ...
    def test_get_all_endpoints(self, mapper):
        ...
        agent_def = AgentDefinition(
            id="test-agent",
            commands=[{"custom_cmd": "Custom command"}]
        )
        mapper.register_agent_mappings(agent_def)
    ...
    def test_create_agent_mapping(self, mapper):
        ...
        mapping = mapper._create_agent_mapping("test-agent", "list_documents", "/api/agents")
        ...
        mapping = mapper._create_agent_mapping("test-agent", "analyze_case", "/api/agents")
        assert mapping.method == HTTPMethod.POST
        assert mapping.websocket_enabled is True
        assert "case_name" in mapping.body_params