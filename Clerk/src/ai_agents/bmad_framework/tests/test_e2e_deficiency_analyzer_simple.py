...
class SimpleE2ETest:
    ...
    def _verify_prerequisites(self) -> Dict[str, Any]:
        ...
        if results["collection_exists"]:
            collection_info = next((case for case in cases if case.get("collection_name") == self.case_name), None)
            if collection_info:
                results["document_count"] = collection_info.get("points_count", 0)
                logger.info(f"Found {results['document_count']} documents in collection")
        else:
            ...
            results["requests"] = [
                {
                    ...
                    "request_text": req.request_text[:200] + "..." if len(req.request_text) > 200 else req.request_text,
    ...
    async def _search_chunks(self, requests: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        results = {
            "search_time_start": datetime.utcnow().isoformat(),
            "searches": []
        }
        ...
        for request in requests:
            ...
            try:
                ...
                query_text = request["request_text"][:500] if len(request["request_text"]) > 500 else request["request_text"]
                query_embedding, _ = self.embedding_generator.generate_embedding(query_text)
                ...
                logger.info(f"Request {request['request_number']}: Searched with truncated query, found {len(chunks)} chunks")
                ...
                for chunk in chunks:
                    ...
                    if hasattr(chunk, 'payload'):
                        chunk_info = {
                            ...
                            "document_name": chunk.payload.get("document_name", "unknown"),
                            "page_number": chunk.payload.get("page_number", "unknown"),
                            "chunk_preview": chunk.payload.get("text", "")[:150] + "..." if len(chunk.payload.get("text", "")) > 150 else chunk.payload.get("text", "")
                        }
                    else:
                        chunk_info = {
                            "score": getattr(chunk, 'relevance_score', 0),
                            "document_name": chunk.get("document_name", "unknown") if isinstance(chunk, dict) else "unknown",
                            "page_number": chunk.get("page_number", "unknown") if isinstance(chunk, dict) else "unknown",
                ...
                doc_names = list(set([
                    c["document_name"] for c in search_result["chunks"]
                ]))
                search_result["unique_documents"] = doc_names
                
                logger.info(f"Request {request['request_number']}: Found {len(chunks)} chunks from {len(doc_names)} documents")
                
            except Exception as e:
                search_result["error"] = str(e)
                logger.error(f"Search failed for request {request['request_number']}: {e}")
    ...
    def _categorize_requests(self, requests: List[Dict[str, Any]], searches: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for request, search in zip(requests, searches):
            ...
            if search.get("error"):
                ...
                categorization["reasoning"] = "No responsive documents found in production"
            elif search.get("chunks_found", 0) < 3:
                categorization["category"] = "PARTIALLY_PRODUCED"
                categorization["confidence"] = 0.7
                categorization["reasoning"] = f"Found {search['chunks_found']} potentially responsive chunks"
            else:
                categorization["category"] = "FULLY_PRODUCED"
                categorization["confidence"] = 0.8
                categorization["reasoning"] = f"Found {search['chunks_found']} responsive chunks from {len(search.get('unique_documents', []))} documents"
            
            results["categorizations"].append(categorization)
            logger.info(f"Request {request['request_number']}: {categorization['category']} (confidence: {categorization['confidence']})")
    ...
    def _save_results(self, results: Dict[str, Any]):
        ...
        # Save JSON results
        json_path = os.path.join(self.output_dir, f"simple_e2e_results_{timestamp}.json")
        with open(json_path, 'w') as f:
            json.dump(results, f, indent=2, default=str)
        
        # Save summary
        summary_path = os.path.join(self.output_dir, f"simple_e2e_summary_{timestamp}.txt")
        with open(summary_path, 'w') as f:
            ...
            if "steps" in results:
                ...
                if "prerequisites" in results["steps"]:
                    ...
                    f.write(f"  - RTP file exists: {prereq.get('rtp_file_exists', False)}\n")
                    f.write(f"  - Collection exists: {prereq.get('collection_exists', False)}\n")
                    f.write(f"  - Document count: {prereq.get('document_count', 0)}\n\n")
                
                # RTP Parsing
                if "rtp_parsing" in results["steps"]:
                    rtp = results["steps"]["rtp_parsing"]
                    f.write(f"RTP Parsing:\n")
                    f.write(f"  - Total requests found: {rtp.get('total_requests_found', 0)}\n")
                ...
                if "categorization" in results["steps"]:
                    ...
                    f.write(f"Categorization Results:\n")
                    for cat in cats:
                        f.write(f"  - Request {cat['request_number']}: {cat['category']} ")
...
async def main():
    ...
    if results["status"] == "SUCCESS":
        ...
        for cat in cats:
            print(f"  Request {cat['request_number']}: {cat['category']} (chunks: {cat['chunks_found']})")
    else:
