...
class BMadDeficiencyAnalyzerE2ETest:
    ...
    async def test_full_bmad_workflow(self) -> Dict[str, Any]:
        ...
        try:
            ...
            search_results = await self._test_bmad_search_command(
                agent_def, analyze_result.get("requests", [])[:3]
            )
            ...
            categorize_results = await self._test_bmad_categorize_command(
                agent_def, analyze_result.get("requests", [])[:3], search_results
            )
    ...
    async def _test_bmad_analyze_command(self, agent_def) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await self.agent_executor.execute_command(
                ...
                parameters={"rtp_path": self.config.rtp_path},
            )
            ...
            return {"status": "failed", "error": str(e), "framework": "BMad"}

    async def _test_bmad_search_command(
        self, agent_def, rtp_requests: List[Dict]
    ) -> Dict[str, Any]:
        """Test search command using BMad framework."""
        results = {"framework": "BMad", "command": "search", "searches": []}
        ...
        for request in rtp_requests[:2]:  # Test first 2 requests
            ...
            try:
                ...
                result = await self.agent_executor.execute_command(
                    ...
                    case_name=self.config.case_name,
                    security_context=self.security_context,
                    parameters={"query": request.get("request_text", ""), "limit": 10},
                )
    ...
    async def _test_bmad_categorize_command(
        self, agent_def, rtp_requests: List[Dict], search_results: Dict
    ) -> Dict[str, Any]:
        """Test categorize command using BMad framework."""
        results = {"framework": "BMad", "command": "categorize", "categorizations": []}
    ...
    async def _test_bmad_full_analysis(self, agent_def) -> Dict[str, Any]:
        ...
        try:
            ...
            if result.success:
                ...
                return {"status": "failed", "error": result.error, "framework": "BMad"}

        except Exception as e:
            return {"status": "error", "error": str(e), "framework": "BMad"}
    ...
    async def _validate_bmad_integration(self, test_results: Dict) -> Dict[str, Any]:
        ...
        validation["all_mappings_valid"] = all(
            validation["mapping_validation"].values()
        )
    ...
    async def _save_outputs(self, test_results: Dict[str, Any]) -> None:
        ...
        results_file = (
            Path(self.config.output_dir) / f"bmad_e2e_test_results_{timestamp}.json"
        )
        ...
        # Save summary report
        summary_file = (
            Path(self.config.output_dir) / f"bmad_e2e_test_summary_{timestamp}.txt"
        )
        with open(summary_file, "w") as f:
            ...
            if "framework_validation" in test_results.get("steps", {}):
                ...
                f.write(
                    f"  - All Mappings Valid: {val.get('all_mappings_valid', False)}\n"
                )
...
async def test_bmad_deficiency_analyzer():
    ...
    assert results["overall_status"] == "success", (
        f"Test failed: {results.get('error')}"
    )
    ...
    assert validation.get("all_mappings_valid", False), (
        "Not all command mappings were valid"
    )
