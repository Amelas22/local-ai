...
class TestExecutionResult:
    ...
    def test_successful_result(self):
        ...
        result = ExecutionResult(success=True, command="test", result={"data": "value"})
...
class TestAgentExecutor:
    ...
    async def test_execute_help_command(
        self, executor, mock_agent_def, mock_security_context
    ):
    ...
    @pytest.mark.asyncio
    async def test_execute_exit_command(
        self, executor, mock_agent_def, mock_security_context
    ):
    ...
    @pytest.mark.asyncio
    async def test_execute_task_command(
        self, executor, mock_agent_def, mock_security_context
    ):
...
"""
    ...
    @pytest.mark.asyncio
    async def test_case_isolation_mismatch(
        self, executor, mock_agent_def, mock_security_context
    ):
        ...
        assert "Case isolation validation failed: Case name mismatch" in str(
            exc_info.value
        )
...
"""
        ...
        with patch.object(
            executor, "_emit_websocket_event", new_callable=AsyncMock
        ) as mock_emit:
            result = await executor.execute_command(
                agent_def=mock_agent_def,
                command="analyze",
...
"""
    ...
    def test_find_task_for_command(self, executor):
        ...
        agent_def = AgentDefinition(
            tasks=["analyze.md", "create-report.md", "search-task.md"]
        )
...
"""
    ...
    @pytest.mark.asyncio
    async def test_task_not_found_error(
        self, executor, mock_agent_def, mock_security_context
    ):
        ...
        assert result.success is False
        assert (
            "Task file not found" in result.error
            or "No task mapping found" in result.error
        )
