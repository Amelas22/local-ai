...
class DeficiencyProgressTracker(ProgressTracker):
    ...
    async def emit_rtp_parsing_progress(
        self, pages_processed: int, total_pages: int, requests_found: int
    ) -> None:
        """
    ...
    async def emit_report_generation_progress(
        self, sections_completed: int, total_sections: int, format_type: str
    ) -> None:
        """
    ...
    async def emit_analysis_completed(
        self, report_id: str, total_deficiencies: int, summary_stats: Dict[str, int]
    ) -> None:
        """
    ...
    def _create_update(
        self, status: str, message: str, metadata: Optional[Dict[str, Any]] = None
    ):
...
async def emit_deficiency_event(
    event_type: str, case_id: str, data: Dict[str, Any]
) -> None:
    """
