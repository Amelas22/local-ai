...
from src.models.fact_models import (
    ...
    SharedKnowledgeEntry, CaseIsolationConfig
)
...
class EnhancedRAGResearchAgent:
    ...
    def __init__(self):
        ...
        # Initialize shared knowledge loaders
        self.florida_statutes_loader = FloridaStatutesLoader()
        self.fmcsr_loader = FMCSRLoader()
...
Create concise, well-attributed summaries that support legal arguments."""
    ...
    async def research(
        ...
        for question in request.questions:
            ...
            optimized_queries = await self._optimize_query(question, request.research_context)
            
            # Search case-specific databases
            if any([request.include_facts, request.include_depositions, request.include_exhibits]):
        ...
        response.research_summary = await self._synthesize_results(all_results, request.questions)
        response.total_results = len(all_results)
        
        # Verify case isolation
        response.case_isolation_verified = self._verify_case_isolation(all_results, request.case_name)
    ...
    async def _search_case_databases(
        ...
        for query in queries:
            ...
            if request.include_facts:
                ...
                for fact in facts:
                    results.append(EnhancedResearchResult(
    ...
    def _organize_results(
        ...
        response.search_strategy["databases_searched"] = list(set(
            [r.source_collection for r in results]
        ))
    ...
    async def _synthesize_results(
        ...
        if "deposition" in by_type:
            synthesis_parts.append(
                ...
                "\n".join([f"- {r.suggested_citation}: {r.content[:150]}..." for r in by_type['deposition'][:3]])
            )
        
        if "exhibit" in by_type:
            synthesis_parts.append(
                f"EXHIBITS ({len(by_type['exhibit'])} found):\n" +
                "\n".join([f"- {r.suggested_citation}: {r.content}" for r in by_type['exhibit'][:3]])
            )
        
        if "statute" in by_type or "regulation" in by_type:
            synthesis_parts.append(
                "LEGAL AUTHORITIES:\n" +
                "\n".join([f"- {r.suggested_citation}: {r.metadata.get('title', '')}" 
                          for r in results if r.source_type in ["statute", "regulation"]][:5])
            )
    ...
    def _verify_case_isolation(
        ...
        for result in results:
            if result.case_name is not None and result.case_name != expected_case:
                logger.error(f"Case isolation violation: Expected {expected_case}, got {result.case_name}")
                return False
        return True


# Create singleton instance for backward compatibility
enhanced_rag_agent = EnhancedRAGResearchAgent()