...
class OutlineCacheManager:
    ...
    async def cache_outline(
        ...
        async with self._lock:
            ...
            logger.info(f"Cached outline {outline_id} for {database_name}: "
    ...
    async def get_section(
        ...
        logger.warning(f"Section index {section_index} out of range for outline {outline_id}")
        return None
    ...
    async def get_section_by_key(
        ...
        async with self._lock:
            ...
            if section_index is None:
                logger.warning(f"Section key '{section_key}' not found in outline {outline_id}")
                return None
    ...
    async def get_outline_structure(self, outline_id: str) -> Optional[List[Dict[str, Any]]]:
        ...
        for i, section in enumerate(sections):
            structure.append({
                "index": i,
                "heading": section.get("heading", section.get("title", f"Section {i+1}")),
    ...
    async def _cleanup_expired(self):
        ...
        expired_ids = [
            oid for oid, cached in self._cache.items()
    ...
    async def get_cache_stats(self) -> Dict[str, Any]:
        ...
        async with self._lock:
            ...
            return {
                ...
                "oldest_outline": min(
                    (c.created_at for c in self._cache.values()),
                    default=None
                ),
                "newest_outline": max(
                    (c.created_at for c in self._cache.values()),
