...
async def draft_motion_background(draft_id: str, request: DraftingRequest):
    ...
    try:
        ...
        draft_progress[draft_id].messages.append("Preparing outline and retrieving case documents")
...
Maintain the same general structure and citations while addressing the revision request."""
                ...
                revised_content = await motion_drafter.section_writer.run(revision_prompt)
        ...
        draft.total_page_estimate = draft.total_word_count // motion_drafter.words_per_page
...
def calculate_quality_metrics(draft) -> QualityMetrics:
    ...
    total_required_citations = sum(
        len(section.outline_section.legal_authorities) 
        for section in draft.sections
    )
    total_found_citations = sum(len(section.citations_used) for section in draft.sections)
    citation_accuracy = min(1.0, total_found_citations / max(total_required_citations, 1))
    
    # Argument strength (based on confidence scores)
    avg_confidence = sum(s.confidence_score for s in draft.sections) / len(draft.sections)
    ...
    # Overall readiness
    readiness = int((draft.coherence_score * 0.4 + citation_accuracy * 0.3 + 
                    avg_confidence * 0.3) * 10)
    ...
    if any(s.needs_revision for s in draft.sections):
        issues.append({
            ...
            "severity": "high",
            "sections": [s.outline_section.id for s in draft.sections if s.needs_revision]
        })
