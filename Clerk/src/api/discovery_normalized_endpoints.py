...
def get_discovery_adapter(
    hierarchical_manager: HierarchicalDocumentManager = Depends(
...
async def list_productions(
    case_id: str,
    producing_party: Optional[str] = Query(
        None, description="Filter by producing party"
    ),
    date_from: Optional[datetime] = Query(
        None, description="Filter by production date from"
    ),
    date_to: Optional[datetime] = Query(
        None, description="Filter by production date to"
    ),
    adapter: DiscoveryNormalizedAdapter = Depends(get_discovery_adapter),
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        for prod in productions.values():
            prod["confidentiality_designations"] = list(
                prod["confidentiality_designations"]
            )
        ...
        for prod in productions.values():
            ...
            if date_from or date_to:
                ...
                if prod_date:
                    prod_date = (
                        datetime.fromisoformat(prod_date)
...
async def check_cross_production_duplicates(
    case_id: str = Body(...),
    check_all_cases: bool = Body(
        False, description="Check across all cases (requires permissions)"
    ),
    hierarchical_manager: HierarchicalDocumentManager = Depends(
        get_hierarchical_manager
    ),
    ...
    try:
        ...
        return {
            ...
            "recommendations": ["No duplicates found across productions"],
        }
