...
if os.getenv("MVP_MODE", "false").lower() == "true":
    ...
    logger.warning("Case endpoints using MVP mode - permissions bypassed")
...
async def create_case_collections_with_events(
    ...
    await emit_case_event(
        "collections_started",
        case_id,
        {"totalCollections": 4, "message": "Creating vector storage collections"},
    )
...
def get_case_context_dependency(required_permission: str = "read"):
    ...
    async def _validate_case_context(
        ...
        # MVP Mode: Return mock case context with full permissions
        if os.getenv("MVP_MODE", "false").lower() == "true":
            mock_context = get_mock_case_context(case_id=case_id)
            # Convert to proper CaseContext model
            return CaseContext(
                case_id=mock_context.case_id,
                case_name=mock_context.case_name,
                user_id=mock_context.user_id,
                permissions=list(mock_context.permissions.keys()),
            )
    ...
    try:
        ...
        for case in cases:
            ...
            case_model = Case(
                ...
                or case.created_at,  # Fallback to created_at if updated_at is NULL
            )
            case_models.append(case_model)

        return CaseListResponse(
            cases=case_models, total=len(case_models), has_more=False
        )
    ...
    try:
        ...
        try:
            ...
            collections_created = await create_case_collections_with_events(
                vector_store, case.collection_name, case.id
            )

            if not collections_created:
                logger.warning(f"Some collections failed to create for case {case.id}")

        except Exception as e:
            # Log error but don't fail case creation
            logger.error(f"Failed to create Qdrant collections for case {case.id}: {e}")
            # Emit error event
            try:
                from src.websocket.socket_server import emit_case_event
