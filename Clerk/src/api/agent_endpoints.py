...
class CategorizeResponse(BaseModel):
    ...
    classification: str = Field(..., pattern="^(fully_produced|partially_produced|not_produced|no_responsive_docs)$")
...
async def analyze_production(
    ...
    try:
        ...
        if "analyze" not in [cmd.split(":")[0] for cmd in agent_def.get("commands", [])]:
            raise HTTPException(
                status_code=400,
                detail={
                    "error": {
        ...
        async with track_progress(
            ...
            await executor.execute_command_async(
                ...
                parameters={
                    ...
                    "oc_response_id": str(body.oc_response_id) if body.oc_response_id else None,
                    "options": body.options,
                    "processing_id": str(processing_id)
                }
            )
        
        logger.info(f"Started analysis {processing_id} for case {case_context.case_name}")
...
async def get_report(
    ...
    try:
        ...
        if format == "pdf":
            headers["Content-Disposition"] = f'attachment; filename="deficiency_report_{report_id}.pdf"'
