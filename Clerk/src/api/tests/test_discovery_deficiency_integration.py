...
class TestFullDiscoveryFlow:
    ...
    @pytest.mark.asyncio
    async def test_complete_discovery_flow_with_deficiency(self, integration_pdf_content):
        ...
        with patch('src.api.discovery_endpoints.emit_discovery_started', new_callable=AsyncMock):
            with patch('src.api.discovery_endpoints.DiscoveryProductionProcessor') as mock_processor_class:
                # Mock discovery processor
                mock_processor = Mock()
                mock_processor.process_discovery_production = AsyncMock()
                mock_processor.process_discovery_production.return_value = mock_discovery_result()
                mock_processor_class.return_value = mock_processor
                
                with patch('src.api.discovery_endpoints.UnifiedDocumentManager') as mock_doc_manager:
                    # Mock document manager
                    mock_manager = Mock()
                    mock_manager.calculate_document_hash = Mock(return_value="test-hash-123")
                    ...
                    with patch('src.api.discovery_endpoints.extract_text_from_pages') as mock_extract:
                        mock_extract.return_value = "Test document content for discovery"
                        
                        with patch('src.api.discovery_endpoints.QdrantVectorStore') as mock_vector_store:
                            # Mock vector store
                            mock_store = Mock()
                            mock_store.store_document_chunks = Mock(return_value=["chunk1", "chunk2"])
                            mock_vector_store.return_value = mock_store
                            
                            with patch('src.api.discovery_endpoints.FactExtractor') as mock_fact_extractor:
                                # Mock fact extractor
                                mock_extractor = Mock()
                                mock_facts = Mock()
                                mock_facts.facts = []
    ...
    async def test_websocket_event_sequence(self):
        ...
        with patch('src.api.discovery_endpoints.sio.emit', side_effect=mock_emit):
            with patch('src.api.discovery_endpoints.emit_discovery_started', new_callable=AsyncMock) as mock_start:
                with patch('src.api.discovery_endpoints.emit_document_found', new_callable=AsyncMock):
                    with patch('src.api.discovery_endpoints.DiscoveryProductionProcessor'):
