...
class TestDiscoveryProcessingE2E:
    ...
    def mock_qdrant(self):
        ...
        with patch(
            "src.vector_storage.qdrant_store.QdrantClient"
    ...
    def mock_openai(self):
        ...
        with (
            patch("src.ai_agents.fact_extractor.openai") as mock_openai,
            patch("src.vector_storage.embeddings.openai") as mock_openai_embed,
            patch(
                "src.document_processing.discovery_splitter.OpenAI"
            ) as mock_openai_splitter,
            patch(
                "src.document_processing.discovery_splitter.pdfplumber"
            ) as mock_pdfplumber,
            ...
            patch(
                "src.document_processing.pdf_extractor.PDFExtractor"
            ) as mock_pdf_extractor,
            patch(
                "src.document_processing.enhanced_chunker.EmbeddingGenerator"
            ) as mock_chunker_embed,
        ):
            ...
            mock_embedding_result = MagicMock(data=[MagicMock(embedding=[0.1] * 1536)])
            mock_openai_embed.Embedding.create = MagicMock(
                return_value=mock_embedding_result
            )
            # Also mock the async version if it exists
            mock_openai_embed.embeddings.create = AsyncMock(
                return_value=mock_embedding_result
            )
            ...
            mock_embed_instance.generate_embeddings_batch_async = AsyncMock(
                return_value=(
                    [[0.1] * 1536],
                    100,
                )  # Returns (embeddings_list, token_count)
            )
            ...
            mock_pdf_reader.pages = [
                MagicMock() for _ in range(38)
            ]  # 38 pages like the real PDF
    ...
    async def app_client(
        self, mock_postgres, mock_qdrant, mock_websocket, mock_openai, mock_case_context
    ):
    ...
    async def test_complete_discovery_processing_flow(
        ...
        response = app_client.post(
            ...
            json=request_data,
            headers={"X-Case-ID": "test-case-id", "Content-Type": "application/json"},
        )
        ...
        document_found_events = [
            e
            for e in mock_websocket.events_captured
    ...
    async def test_discovery_error_handling(
        ...
        with patch(
            "src.document_processing.discovery_splitter.OpenAI"
        ) as mock_openai_error:
            mock_openai_error.return_value.chat.completions.create.side_effect = (
                ...
                "discovery_files": [
                    {
                        ...
                        "content": base64.b64encode(b"This is not a valid PDF").decode(
                            "utf-8"
                        ),
                        "content_type": "application/pdf",
                    }
    ...
    async def test_fact_search_after_processing(
        self, app_client, test_case_name, mock_postgres, mock_case_context
    ):
            ...
            data=[
                {
                    ...
                    "source_metadata": {"document_id": "doc-1", "page": 3},
                },
                {
                    "id": "fact-2",
                    ...
                    "confidence": 0.95,
                    "case_name": test_case_name,
                    "source_metadata": {"document_id": "doc-1", "page": 5},
                },
            ]
        )
    ...
    async def test_document_deduplication(
        ...
        with patch(
            "src.document_processing.unified_document_manager.UnifiedDocumentManager.is_duplicate"
        ) as mock_is_dup:
