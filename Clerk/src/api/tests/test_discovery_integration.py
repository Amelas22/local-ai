...
class TestDiscoveryIntegration:
    ...
    def test_process_discovery_json_request(self, client, sample_pdf_base64):
        ...
        with (
            patch(
                "src.api.discovery_endpoints.require_case_context"
            ) as mock_require_context,
            ...
            request_data = {
                "discovery_files": [
                    {"filename": "test_discovery.pdf", "content": sample_pdf_base64}
                ],
            ...
            response = client.post(
                "/api/discovery/process", json=request_data, headers=headers
            )
    ...
    def test_get_processing_status_integration(self, client):
        ...
        with patch("src.api.discovery_endpoints.get_case_context") as mock_get_context:
            mock_get_context.return_value = Mock(
                case_id="test-case-1", case_name="Test Case"
            )

            headers = {"X-Case-ID": "test-case-1"}
            response = client.get(
                f"/api/discovery/status/{processing_id}", headers=headers
            )
