...
class TestDiscoveryEndpoints:
    ...
    def mock_dependencies(self):
        ...
        with (
            patch("src.api.discovery_endpoints.vector_store") as mock_vector_store,
            patch("src.api.discovery_endpoints.sio") as mock_sio,
            patch("src.api.discovery_endpoints.FactExtractor") as mock_fact_extractor,
            patch(
                "src.api.discovery_endpoints.NormalizedDiscoveryProductionProcessor"
    ...
    @pytest.mark.asyncio
    async def test_start_discovery_processing_with_file_upload(
        self, client, mock_dependencies, sample_pdf_content
    ):
        ...
        mock_pdf_extractor_instance.extract_text_from_pages = Mock(
        ...
        mock_chunker_instance.create_chunks_with_context = Mock(
            return_value=[
        ...
        mock_fact_extractor_instance.extract_facts_from_document = AsyncMock(
            return_value=self.sample_facts()
        )
    ...
    @pytest.mark.asyncio
    async def test_discovery_processing_with_document_splitting(
        ...
        discovery_files = [{"filename": "test.pdf", "content": b"PDF content"}]
    ...
    def _setup_processing_mocks(self, mock_dependencies):
        ...
        mock_processor.process_production_normalized = AsyncMock(
            return_value=DiscoveryProductionResult(
                segments_found=self.sample_discovery_segments(),
                total_pages=20,
                processing_windows=4,
        ...
        mock_chunker.create_chunks_with_context = Mock(
            return_value=[Mock(chunk_id="chunk1", text="text", metadata={})]
        )
...
class TestDiscoveryStatusEndpoint:
    ...
    async def test_get_discovery_status(self, client):
        ...
        with patch(
            "src.api.discovery_endpoints.get_processing_status"
        ) as mock_get_status:
            ...
            response = client.get(
                "/api/discovery/status/test_123", headers={"X-Case-ID": "test_case"}
            )
