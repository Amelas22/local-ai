...
from src.api.discovery_endpoints import (
    router,
    process_discovery_with_deficiency,
    _process_discovery_core,
    _process_discovery_with_cleanup,
    processing_status
)
from src.models.discovery_models import (
    DiscoveryProcessWithDeficiencyRequest,
    DiscoveryProcessingResponse,
    DiscoveryProcessingStatus
)
from src.utils.temp_file_manager import TempFileManager, get_temp_file_manager
from src.utils.pdf_validator import validate_pdf_content
...
class TestDiscoveryWithDeficiencyEndpoint:
    ...
    @pytest.mark.asyncio
    async def test_process_with_valid_files(self, test_client, mock_case_context, sample_discovery_request):
        ...
        with patch('src.api.discovery_endpoints.require_case_context') as mock_require:
            ...
            with patch('src.api.discovery_endpoints.BackgroundTasks.add_task'):
                ...
                with patch('src.api.discovery_endpoints.sio.emit', new_callable=AsyncMock):
                    with patch('src.api.discovery_endpoints.emit_discovery_started', new_callable=AsyncMock):
    ...
    @pytest.mark.asyncio
    async def test_process_without_rtp_oc_files(self, test_client, mock_case_context, valid_pdf_content):
    ...
    def test_invalid_discovery_pdf(self, test_client, mock_case_context, invalid_pdf_content):
    ...
    def test_invalid_rtp_file(self, test_client, mock_case_context, valid_pdf_content, invalid_pdf_content):
    ...
    def test_oversized_file(self, test_client, mock_case_context, oversized_pdf_content):
    ...
    @pytest.mark.asyncio
    async def test_multipart_form_upload(self, test_client, mock_case_context, valid_pdf_content):
        ...
        with patch('src.api.discovery_endpoints.require_case_context') as mock_require:
            ...
            with patch('src.api.discovery_endpoints.Request.form') as mock_form:
                ...
                mock_form.return_value = AsyncMock(return_value=Mock(
...
class TestWebSocketEvents:
    ...
    @pytest.mark.asyncio
    async def test_rtp_upload_event(self, test_client, mock_case_context, sample_discovery_request):
        """Test that RTP upload triggers WebSocket event."""
        with patch('src.api.discovery_endpoints.require_case_context') as mock_require:
            mock_require.return_value = lambda: mock_case_context
            
            with patch('src.api.discovery_endpoints.BackgroundTasks.add_task'):
                with patch('src.api.discovery_endpoints.sio.emit', new_callable=AsyncMock) as mock_emit:
                    with patch('src.api.discovery_endpoints.emit_discovery_started', new_callable=AsyncMock):
                        response = test_client.post(
                            "/api/discovery/process-with-deficiency",
                            json=sample_discovery_request,
    ...
    @pytest.mark.asyncio
    async def test_oc_response_upload_event(self, test_client, mock_case_context, sample_discovery_request):
...
class TestCoreProcessingLogic:
    ...
    async def test_core_processing_with_metadata(self, valid_pdf_content):
        ...
        # Mock dependencies
        with patch('src.api.discovery_endpoints.emit_discovery_started', new_callable=AsyncMock):
            with patch('src.api.discovery_endpoints.DiscoveryProductionProcessor') as mock_processor:
                mock_instance = Mock()
                mock_instance.process_discovery_production = AsyncMock(
                    return_value=Mock(
                        segments_found=[],
                        average_confidence=0.9,
                        processing_windows=5
                    )
                )
    ...
    async def test_cleanup_wrapper_function(self, valid_pdf_content):
        ...
        with patch('src.api.discovery_endpoints._process_discovery_core', new_callable=AsyncMock) as mock_core:
            with patch('src.api.discovery_endpoints.get_temp_file_manager') as mock_get_manager:
                mock_manager = Mock()
...
class TestBackwardCompatibility:
    ...
    def test_existing_endpoint_unchanged(self, test_client, mock_case_context, valid_pdf_content):
