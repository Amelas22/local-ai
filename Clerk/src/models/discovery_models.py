...
class DiscoveryProcessingRequest(BaseModel):
    ...
    discovery_files: List[str] = Field(default_factory=list, description="Files to process")
    ...
    max_facts_per_document: Optional[int] = Field(None, description="Limit facts per document")
...
class DiscoveryProcessingStatus(BaseModel):
    ...
    status: str = Field(default="initializing")  # initializing, processing, completed, error
