...
def mock_case_manager():
    ...
    with patch("src.middleware.case_context.case_manager") as mock:
        ...
        mock.get_case_context = AsyncMock(
            return_value=CaseContext(
                case_id="case-123",
                case_name="Test Case",
...
class TestCaseContextMiddleware:
    ...
    @pytest.mark.asyncio
    async def test_case_id_from_header(
        self, middleware, mock_request, mock_case_manager
    ):
    ...
    @pytest.mark.asyncio
    async def test_case_id_from_query_params(
        self, middleware, mock_request, mock_case_manager
    ):
        ...
        mock_case_manager.validate_case_access.assert_called_once_with(
            case_id="case-456", user_id="user-123", required_permission="read"
        )
    ...
    @pytest.mark.asyncio
    async def test_case_not_found_returns_404(
        self, middleware, mock_request, mock_case_manager
    ):
    ...
    @pytest.mark.asyncio
    async def test_successful_case_context_injection(
        self, middleware, mock_request, mock_case_manager
    ):
