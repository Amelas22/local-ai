...
class CaseService:
    ...
    async def create_case(
        ...
        # Log case creation
        log_case_access(
            logger=logger, user_id=created_by, case_name=name, action="case_created"
        )
    ...
    async def get_user_cases(
        ...
        query = (
            ...
            .options(selectinload(Case.law_firm), selectinload(Case.permissions))
        )
    ...
    async def get_case_by_name(
        db: AsyncSession, name: str, law_firm_id: str
    ) -> Optional[Case]:
        """
    ...
    async def update_case_status(
        db: AsyncSession, case_id: str, status: CaseStatus, user_id: str
    ) -> Optional[Case]:
        """
        Update case status.
    ...
    async def validate_case_access(
        db: AsyncSession, case_id: str, user_id: str, required_permission: str = "read"
    ) -> bool:
        """
        ...
        """
        result = await db.execute(
            select(UserCasePermission).where(
                and_(
    ...
    async def grant_case_permission(
        ...
        if not await CaseService.validate_case_access(
            db, case_id, granting_user_id, "admin"
        ):
