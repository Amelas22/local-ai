...
class TestCaseNameToCollection:
    ...
    def test_special_characters_removal(self, case_manager_instance):
        ...
        result = case_manager_instance.case_name_to_collection(
            "Smith & Jones, LLC vs. ABC Corp.", "law-firm-123"
        )
...
class TestCreateCase:
    ...
    async def test_create_case_success(
        self, case_manager_instance, mock_supabase_client
    ):
        ...
        with patch.object(case_manager_instance, "get_case_by_name", return_value=None):
            with patch.object(
                case_manager_instance, "_grant_case_permission", return_value=True
            ):
                result = await case_manager_instance.create_case(
                    name="Test Case", law_firm_id="firm-123", created_by="user-123"
                )
    ...
    async def test_create_case_empty_name(self, case_manager_instance):
        ...
        with pytest.raises(ValueError, match="Case name cannot be empty"):
            await case_manager_instance.create_case(
                name="", law_firm_id="firm-123", created_by="user-123"
            )
    ...
    async def test_create_case_name_too_long(self, case_manager_instance):
        ...
        with pytest.raises(ValueError, match="Case name too long"):
            await case_manager_instance.create_case(
                name=long_name, law_firm_id="firm-123", created_by="user-123"
            )
    ...
    async def test_create_case_duplicate_name(self, case_manager_instance):
        ...
        with patch.object(
            case_manager_instance, "get_case_by_name", return_value=existing_case
        ):
            with pytest.raises(ValueError, match="already exists"):
                await case_manager_instance.create_case(
                    name="Existing Case", law_firm_id="firm-123", created_by="user-123"
                )
...
class TestGetUserCases:
    ...
    @pytest.mark.asyncio
    async def test_get_user_cases_success(
        self, case_manager_instance, mock_supabase_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_user_cases_with_filters(
        self, case_manager_instance, mock_supabase_client
    ):
        ...
        await case_manager_instance.get_user_cases(
            user_id="user-123", law_firm_id="firm-123", status=CaseStatus.ACTIVE
        )
    ...
    @pytest.mark.asyncio
    async def test_get_user_cases_error_handling(
        self, case_manager_instance, mock_supabase_client
    ):
...
class TestValidateCaseAccess:
    ...
    @pytest.mark.asyncio
    async def test_validate_access_granted(
        self, case_manager_instance, mock_supabase_client
    ):
        ...
        result = await case_manager_instance.validate_case_access(
            case_id="case-123", user_id="user-123", required_permission="read"
        )

        assert result is True

    @pytest.mark.asyncio
    async def test_validate_access_denied_no_permission(
        self, case_manager_instance, mock_supabase_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_validate_access_expired_permission(
        self, case_manager_instance, mock_supabase_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_validate_access_permission_hierarchy(
        self, case_manager_instance, mock_supabase_client
    ):
        ...
        result = await case_manager_instance.validate_case_access(
            case_id="case-123", user_id="user-123", required_permission="write"
        )
...
class TestGetCaseContext:
    ...
    @pytest.mark.asyncio
    async def test_get_case_context_success(
        self, case_manager_instance, mock_supabase_client
    ):
        ...
        case_query.eq.return_value.single.return_value.execute.return_value.data = (
            mock_case
        )
        ...
        perm_query.eq.return_value.eq.return_value.execute.return_value.data = (
            mock_permissions
        )
        mock_supabase_client.table.return_value.select.side_effect = [
            case_query,
        ...
        result = await case_manager_instance.get_case_context(
            case_id="case-123", user_id="user-123"
        )
...
class TestSupabaseInitializationRetry:
    ...
    def test_initialization_retry_success_second_attempt(
        self, mock_sleep, mock_settings, mock_create_client
    ):
    ...
    @patch("src.services.case_manager.settings")
    @patch("src.services.case_manager.time.sleep")
    def test_initialization_retry_all_attempts_fail(
        self, mock_sleep, mock_settings, mock_create_client
    ):
    ...
    @patch("src.services.case_manager.create_client")
    @patch("src.services.case_manager.settings")
    def test_initialization_success_first_attempt(
    ...
    def test_initialization_logs_retry_attempts(
        self, mock_logger, mock_settings, mock_create_client
    ):
        ...
        mock_logger.error.assert_called_with(
            "Failed to initialize Supabase client after 3 attempts"
        )
