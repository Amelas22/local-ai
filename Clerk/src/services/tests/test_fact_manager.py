...
def mock_dependencies():
    ...
    with patch('src.services.fact_manager.QdrantVectorStore') as mock_qdrant, \
...
class TestFactManager:
    ...
    @pytest.mark.asyncio
    async def test_create_fact_success(self, mock_dependencies, sample_fact, case_context):
    ...
    @pytest.mark.asyncio
    async def test_create_fact_duplicate_detection(self, mock_dependencies, sample_fact, case_context):
    ...
    @pytest.mark.asyncio
    async def test_create_fact_case_mismatch(self, mock_dependencies, sample_fact, case_context):
    ...
    async def test_update_fact_success(self, mock_dependencies, case_context):
        ...
        mock_dependencies['embedding'].generate_embedding.assert_called_with("Updated content")
        mock_dependencies['qdrant'].client.upsert.assert_called_once()
    ...
    async def test_delete_fact_success(self, mock_dependencies, case_context):
        ...
        delete_request = FactDeleteRequest(
            fact_id=fact_id,
            delete_reason="Incorrect information"
        )
    ...
    async def test_search_facts_with_query(self, mock_dependencies, case_context):
        ...
        mock_dependencies['embedding'].generate_embedding.assert_called_with("back pain")
