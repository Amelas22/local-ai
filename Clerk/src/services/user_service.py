...
class UserService:
    ...
    async def create_user(
        ...
        # Load relationships
        await db.execute(
            select(User).options(selectinload(User.law_firm)).where(User.id == user.id)
        )
    ...
    async def get_users_by_law_firm(db: AsyncSession, law_firm_id: str) -> List[User]:
        ...
        result = await db.execute(
            select(User)
            .where(and_(User.law_firm_id == law_firm_id, User.is_active == True))
            .order_by(User.name)
        )
    ...
    async def create_law_firm(
        db: AsyncSession, name: str, domain: Optional[str] = None
    ) -> LawFirm:
        """
        ...
        existing = await db.execute(select(LawFirm).where(LawFirm.name == name))
        if existing.scalar_one_or_none():
