...
class CaseManager:
    ...
    def _initialize_client(self) -> None:
        ...
        if not settings.supabase.url or not settings.supabase.anon_key:
            logger.warning(
                "Supabase credentials not configured. Case management will not work."
            )
            return
        ...
        for attempt in range(max_retries):
            try:
                logger.info(
                    f"Attempting to initialize Supabase client (attempt {attempt + 1}/{max_retries})"
                )
                ...
                self._client = create_client(
                    settings.supabase.url, auth_key, options=options
                )
                ...
                logger.warning(
                    f"Attempt {attempt + 1} failed to initialize Supabase client: {str(e)}"
                )
                if attempt < max_retries - 1:
                    ...
                    logger.error(
                        f"Failed to initialize Supabase client after {max_retries} attempts"
                    )
    ...
    def case_name_to_collection(self, case_name: str, law_firm_id: str) -> str:
        ...
        sanitized = "".join(
            c if c.isalnum() or c in "_- " else "_" for c in case_name.lower()
        )
    ...
    async def create_case(
        ...
        try:
            ...
            if len(name) > 50:
                raise ValueError(
                    f"Case name too long ({len(name)} chars). Maximum 50 characters allowed."
                )
    ...
    async def get_case_by_name(self, name: str, law_firm_id: str) -> Optional[Case]:
        ...
        try:
            result = (
                ...
                .select("*")
                .eq("name", name)
                .eq("law_firm_id", str(law_firm_id))
                .execute()
            )
    ...
    async def validate_case_access(
        ...
        try:
            result = (
                self.client.table("user_case_permissions")
                .select("*")
            ...
            if permission.get("expires_at"):
                expires_at = datetime.fromisoformat(
                    permission["expires_at"].replace("Z", "+00:00")
                )
