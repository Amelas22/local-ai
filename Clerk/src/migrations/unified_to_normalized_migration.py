...
class UnifiedToNormalizedMigration:
    ...
    async def migrate_all_data(self, dry_run: bool = False) -> MigrationStats:
        ...
        try:
            ...
            self.logger.info(
                f"Found {len(existing_data['cases'])} unique cases with {len(existing_data['documents'])} documents"
            )
            ...
            while scroll_result:
                for point in scroll_result[0]:
                    try:
                        ...
                        self.logger.warning(
                            f"Failed to parse document from {collection_name}: {e}"
                        )
    ...
    async def _analyze_migration_requirements(
        ...
        stats.relationships_created = self._estimate_document_relationships(
        ...
        self.logger.info("Migration analysis complete (dry run)")
        return stats
            ...
            for case_name, case_info in cases_data.items():
                ...
                if " v. " in case_name or " v " in case_name:
                    ...
                    plaintiffs = (
                        [parties[0].strip()]
                        if len(parties) > 0
                        else ["Unknown Plaintiff"]
                    )
                    defendants = (
                        [parties[1].strip()]
                        if len(parties) > 1
                        else ["Unknown Defendant"]
                    )
        ...
        try:
            for doc in documents:
                try:
                    ...
                    if not case_id:
                        self.logger.warning(
                            f"No case found for document: {doc.file_name}"
                        )
                        continue
                    ...
                    # Update mappings
                    self.document_hash_to_id_mapping[doc.document_hash] = (
                        document_core.id
                    )
                    ...
                    if self.stats.documents_migrated % 100 == 0:
                        self.logger.info(
                            f"Migrated {self.stats.documents_migrated} documents..."
                        )

                except Exception as e:
                    self.logger.error(
                        f"Failed to migrate document {doc.file_name}: {e}"
                    )
    ...
    async def _migrate_chunks(self):
        ...
        try:
            ...
            for collection_name in chunk_collections:
                ...
                migrated_chunks = await self._convert_chunks_to_normalized(
                    chunks, case_id
                )
                ...
                self.logger.info(
                    f"Migrated {len(migrated_chunks)} chunks from {collection_name}"
                )
    ...
    async def _find_chunk_collections(self) -> List[str]:
        ...
        return [
            col
            for col in collections
            if not col.endswith("_documents") and "legal" not in col
        ]
    ...
    async def _get_chunks_from_collection(
        self, collection_name: str
    ) -> List[Dict[str, Any]]:
    ...
    async def _convert_chunks_to_normalized(
        self, chunks: List[Dict[str, Any]], case_id: str
    ):
